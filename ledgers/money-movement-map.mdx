...
Money movement map is a group of all <Tooltip tip="How money moves between balances from initiation to its final destination.">transaction journeys</Tooltip> in your system. 
...
In the flowchart above, `@World` represents all external accounts (card, other bank accounts, etc.) interacting with your user's balance. 
...
Fintech A only needs to create one ledger to group all of his users' balances. `@World` and `@Fees` are automatically added to the [General Ledger](general-ledger).
