...
# Private HTTPS repos only (optional)
...
| `WATCH_SCRIPT_GIT_USERNAME` | Username for private HTTPS Git clones. If you embed credentials in the repo URL, Watch extracts them and keeps the stored URL clean in logs and status responses. | `x-access-token` | No |
| `WATCH_SCRIPT_GIT_TOKEN` | Personal access token or password used with `WATCH_SCRIPT_GIT_USERNAME` for private HTTPS repositories. Store this as a secret. | None | No |

<Note>
Git-based rule loading requires `git` to be installed on the machine running Watch. For private HTTPS repositories, set `WATCH_SCRIPT_GIT_TOKEN` (and optionally `WATCH_SCRIPT_GIT_USERNAME`). For SSH URLs, load the appropriate SSH key on the host instead.
</Note>
...
<RelatedTopics
  items={[
    ...
    { title: "Webhooks", href: "/watch/webhooks" },
  ]}
/>
