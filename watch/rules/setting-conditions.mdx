...
| `in` | value is in a list | `meta_data.channel in`<br />`("card", "bank_transfer")` |
| `regex` | matches a regular expression | `description regex`<br />`"(?i)(gift.?card\|crypto)"` |
| `not_regex` | does not match a regular expression | `description not_regex`<br />`"(?i)test"` |
...
rule lateNightLargeTransfer {
  when hour_of_day(created_at) >= 22 
    and amount > 3000
...
Time-based functions read from **`created_at`** on the transaction: the value you send on inject or that Watch syncs from Blnk Core.

You can write time-based conditions using helper functions that extract parts of the timestamp.

| Function | Description | Example |
| --- | --- | --- |
| `hour_of_day(created_at)` | Hour of day (0-23) | `hour_of_day(created_at) >= 22` |
| `day_of_week(created_at)` | Weekday: 0 = Sunday, 6 = Saturday. With `in`, you can use day names | `day_of_week(created_at) in (0, 6)` or `day_of_week(created_at) in`<br />`("Saturday", "Sunday")` |
| `day_of_month(created_at)` | Day of month (1-31) | `day_of_month(created_at) == 17` |
| `day_of_year(created_at)` | Day of year (1-366) | `day_of_year(created_at) <= 31` |
| `month_of_year(created_at)` | Month (1-12) | `month_of_year(created_at) == 3` |
| `week_of_year(created_at)` | ISO week number (1-53) | `week_of_year(created_at) == 12` |
| `year(created_at)` | Year (e.g. 2026) | `year(created_at) == 2026` |

> **Also note:** Time-based conditions support standard comparison operators: `==`, `!=`, `>`, `>=`, `<`, `<=`, `in`, and `not_in`.

> For `day_of_week`, when using the `in` operator, you can use day names instead of numbers. So `day_of_week(created_at) in ("Saturday", "Sunday")` is equivalent to `day_of_week(created_at) in (0, 6)`.
...
>- The lookback window ends at the **current transaction's `created_at`**, not server time.
>- `match` keys must be **table columns** (`source`, `destination`, `status`, and so on) or **metadata paths** (`meta_data.status`, `metadata.status`). Dotted keys in object literals are supported (for example `meta_data.status: "failed"`).
