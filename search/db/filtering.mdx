...
results, resp, err := client.Transaction.Filter(blnkgo.FilterParams{
  ...
  Filters: []blnkgo.Filter{
    ...
    {Field: "currency", Operator: blnkgo.OpIn, Values: []string{"USD", "EUR"}},
  },
  SortBy:       "created_at",
  SortOrder:    "desc",
  IncludeCount: true,
...
results, resp, err := client.Transaction.Filter(blnkgo.FilterParams{
  Filters: []blnkgo.Filter{
    {Field: "meta_data.customer_type", Operator: blnkgo.OpEqual, Value: "premium"},
  },
})
```
</CodeGroup>
  ...
  "data": [
    {
      ...
      "meta_data": { "customer_type": "premium" }
    }
  ]
}
```
