...
icon: "book-open"
description: "Use Typesense for full-text search in Blnk. Enable it by setting up your Typesense configuration."
"og:title": "Search via Typesense • Blnk Developer Documentation" 
"og:description": "Use Typesense for full-text search in Blnk. Enable it by setting up your Typesense configuration."
---
...
Blnk supports Typesense for search and indexing. You can use it to retrieve ledgers, balances, transactions, and identities with filtering, sorting, faceting, and full-text search.

To enable Typesense search and indexing, set up your [Typesense configuration](/advanced/configuration/data-stores#typesense-configuration). Until you do, Typesense search and indexing stay disabled by default.

Once Typesense is enabled, use the Search API when you need to:
...
| Several collections | `/multi-search` | Run more than one of the searches above in one request. See [Search via multi-search](/reference/search-multi-search). |
...
results, resp, err := client.Search.SearchDocument(
    blnkgo.SearchParams{
        Q: "*",
        FilterBy: "created_at:[1704067200..1706745599]",
        SortBy: "created_at:desc",
    },
    blnkgo.Transactions,
)
...
ApiResponse<JsonNode> response = blnk.search().search(
    SearchParams.create()
        .q("*")
        .filterBy(
            "created_at:[1704067200..1706745599]"
        )
        .sortBy("created_at:desc"),
    "transactions"
);
...
results, resp, err := client.Search.SearchDocument(
    blnkgo.SearchParams{
        Q: "payment",
        QueryBy: "meta_data.transaction_type,description",
        SortBy: "created_at:desc",
    },
    blnkgo.Transactions,
)
...
curl -X POST "http://YOUR_BLNK_INSTANCE_URL/search/transactions" \
  ...
  -d '{
    "q": "*",
    "filter_by": "source:=bln_28f25ef6-2e0d-4fa6-891c-37fc409d654e && '\
...
```go Go wrap
filterBy := "source:=bln_28f25ef6-2e0d-4fa6-891c-37fc409d654e" +
    " && destination:=bln_86ba7976-499d-4282-955e-a7c2abf5db12"
...
results, resp, err := client.Search.SearchDocument(
    blnkgo.SearchParams{
        Q: "john doe",
        QueryBy: "first_name,last_name,email",
        SortBy: "created_at:desc",
    },
    blnkgo.Identities,
)
...
<RelatedTopics
  items={[
    { title: "Typesense configuration", href: "/advanced/configuration/data-stores#typesense-configuration" },
