...
| `BLNK_TRANSACTION_MAX_QUEUE_SIZE` | Maximum number of transactions that can be queued for processing. | `1000` |
| `BLNK_TRANSACTION_MAX_WORKERS` | Maximum number of concurrent workers processing transactions. | `10` |
| `BLNK_TRANSACTION_LOCK_DURATION` | How long a distributed balance lock can live once acquired. | `30m` |
| `BLNK_TRANSACTION_LOCK_WAIT_TIMEOUT` | How long synchronous transactions wait and retry when a required balance lock is busy. Applies to `skip_queue=true`. | `3s` |
| `BLNK_TRANSACTION_INDEX_QUEUE_PREFIX` | Prefix used for transaction queue names in the indexing system. | `transactions` |
...
This controls how Blnk behaves when a transaction bypasses the queue, i.e. `skip_queue=true` cannot acquire its required lock immediately.

It tells Blnk how long to wait for the lock to be acquired. If the lock is not acquired within this time, the transaction is rejected.
...
| `BLNK_TRANSACTION_DISABLE_BATCH_REFERENCE_CHECK` | Disables reference validation across a coalesced batch before commit. When `false`, reference checking stays enabled. | `true` |
...
See instead: [How to handle hot balances](/guides/hot-balances).
...
This sets the maximum number of times Blnk will try a queued transaction again after a temporary failure such as lock content, insufficient funds, etc.
...
| `BLNK_QUEUE_HOT_PAIR_TTL` | How long hot-pair activity and contention state is remembered. | `5m` |
