...
| `BLNK_TRANSACTION_DISABLE_BATCH_REFERENCE_CHECK` | Skips the bulk reference lookup during coalesced batch processing. References are still validated per transaction. When `false`, the bulk check stays enabled. | `false` |
