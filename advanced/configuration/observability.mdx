...
| `BLNK_ENABLE_TELEMETRY` | When `true`, Blnk sends **product heartbeat / usage** telemetry (for example to PostHog) so the team can understand adoption and health trends. It is **not** the same switch as distributed traces or `/metrics`. | `false` |
