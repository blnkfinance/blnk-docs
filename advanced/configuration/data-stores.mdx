...
<Note>
In `blnk.json`, `conn_max_lifetime` and `conn_max_idle_time` are integer seconds. Environment variables accept Go duration format (e.g. `30m`, `5m`).
</Note>

<CodeGroup>
```bash blnk.env
BLNK_DATA_SOURCE_DNS="postgres://user:pass@postgres:5432/blnk?sslmode=disable"
BLNK_DATABASE_MAX_OPEN_CONNS=50
BLNK_DATABASE_MAX_IDLE_CONNS=25
...
| `BLNK_DATABASE_MAX_OPEN_CONNS`     | Maximum number of open database connections in the pool.                          | `50`    |
| `BLNK_DATABASE_MAX_IDLE_CONNS`     | Maximum number of idle database connections kept ready.                           | `25`    |
...
From 0.15.4, Blnk retries the initial database connection instead of exiting on the first failure.
...
<Note>
In `blnk.json`, `conn_max_idle_time` is a duration in nanoseconds (for example `120000000000` for 2m). Environment variables accept Go duration format (for example `2m`).
</Note>
...
BLNK_REDIS_CONN_MAX_IDLE_TIME=2m
```
...
```json blnk.json
{
  "redis": {
    ...
    "pool_size": 100,
    "min_idle_conns": 20,
    "conn_max_idle_time": 120000000000
  }
}
```
</CodeGroup>
...
| `BLNK_REDIS_POOL_SIZE`         | Maximum connections in the Redis pool.                                                              | `100`   |
| `BLNK_REDIS_MIN_IDLE_CONNS`    | Idle connections kept ready in the pool.                                                            | `20`    |
| `BLNK_REDIS_CONN_MAX_IDLE_TIME` | How long a pooled connection may sit idle before Blnk discards it. Env: Go duration (`2m`). JSON: nanoseconds. From 0.15.5. | `2m`    |
...
### `BLNK_REDIS_CONN_MAX_IDLE_TIME`

Available in version 0.15.5 and later.

How long a pooled Redis or Asynq connection may sit idle before Blnk discards it instead of reusing it. Applies to Redis cache clients and Asynq queue workers. The default is `2m`. Leave it unset unless your Redis, Valkey, or proxy idle timeout is shorter than that.

Set this **below** the server or proxy idle timeout (managed services often use about 300 seconds). If a connection stays idle longer than the remote timeout, the next command can fail with `write: broken pipe`.
...
<Note>
In `blnk.json`, `backpressure_retry_interval` is a duration in **nanoseconds** (for example `5000000000` for 5s). The environment variable accepts Go duration format (for example `5s`). Omit the field to use the default.
</Note>
...
BLNK_TYPESENSE_BACKPRESSURE_RETRY_INTERVAL=5s
```
...
```json blnk.json
{
  "typesense": {
    ...
    "backpressure_retry_interval": 5000000000
  },
...
| `BLNK_TYPESENSE_BACKPRESSURE_RETRY_INTERVAL` | How long Blnk waits before retrying indexing or reindex work after Typesense reports a memory limit error. | `5s` |
| `TYPESENSE_MEMORY_USED_MAX_PERCENTAGE` | Typesense container setting. Caps Typesense memory usage as a percentage of available memory before it rejects writes. | `85` |

Set Typesense memory limits on the Typesense service itself, not in Blnk:

```bash typesense.env
TYPESENSE_MEMORY_USED_MAX_PERCENTAGE=85
```
...
<RelatedTopics
  items={[
    ...
    { title: "Backup", href: "/advanced/configuration/backup" },
    { title: "Observability", href: "/advanced/configuration/observability" },
  ]}
/>
