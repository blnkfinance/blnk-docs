...
When [observability](/advanced/observability) is enabled, Blnk serves Prometheus metrics at `GET /metrics` on the API port and on the [worker monitoring port](/advanced/monitoring-port). Set `metrics_bearer_token` to require `Authorization: Bearer <token>` on scrape requests.
