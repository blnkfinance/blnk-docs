...
| `BLNK_SERVER_PORT` | The port Blnk listens on for incoming HTTP or HTTPS requests. | `5001` |
...
Use these settings to serve the API over HTTPS. TLS is off by default.

<Info>
Honoured again from Blnk Core 0.15.4. Through 0.12.x the flag worked. From 0.13.0 through 0.15.3 it was ignored and the server always served plaintext HTTP. See the [0.15.4 migration guide](/changelog/v15-4-migration#serverssl-starts-https).
</Info>
...
| | Description | Default |
| :-- | :-- | :-- |
| `BLNK_SERVER_SSL` | When `true`, Blnk starts an HTTPS listener on `server.port` and obtains a certificate for `server.domain`. When `false`, Blnk serves plaintext HTTP. | `false` |
| `BLNK_SERVER_SSL_DOMAIN` | Domain used to issue the certificate. Point this hostname at the server before you enable SSL. When empty, Blnk defaults to `localhost`. | `localhost` |
| `BLNK_SERVER_SSL_EMAIL` | Email Let's Encrypt uses for certificate expiry notices. | None |
| `BLNK_CERT_STORAGE_PATH` | Directory where issued certificates are stored. The process must be able to write here. | `/var/lib/blnk/certs` |

### How `server.ssl` works

When `ssl` is `false`, Blnk serves plaintext HTTP on `server.port`.

When `ssl` is `true`, Blnk:

1. Starts an HTTPS listener on the same `server.port` (not a separate TLS port).
2. Obtains a certificate for `server.domain` with Let's Encrypt (certmagic).
3. Writes certificates to `cert_storage_path`.
4. Logs the scheme (`https`) on startup so you can confirm TLS is on.
5. Uses the same graceful shutdown path as the HTTP listener.

If a certificate cannot be issued, the server **fails to start**. It does not fall back to plaintext HTTP. A certificate failure names the domains in the error instead of exiting from a background goroutine.

<Warning>
If you left `"ssl": true` in `blnk.json` (or `BLNK_SERVER_SSL=true`) while your clients still use `http://`, those clients will fail the TLS handshake after you upgrade to 0.15.4. Set `ssl` to `false` to keep HTTP, or finish the HTTPS setup before you deploy.
</Warning>

<Card title="Enable HTTPS" icon="lock" href="/advanced/enable-https">
  Domain, certificate, and verification steps.
</Card>
...
| :-- | :-- | :-- |
| JSON API requests | `BLNK_SERVER_MAX_REQUEST_BODY_SIZE_MB` | `400` with `GEN_MALFORMED_REQUEST` and message `http: request body too large`. |
| Multipart uploads | `BLNK_SERVER_MAX_UPLOAD_SIZE_MB` | `413` with `GEN_PAYLOAD_TOO_LARGE` on reconciliation file upload. |
...
<RelatedTopics
  items={[
    { title: "Enable HTTPS", href: "/advanced/enable-https" },
