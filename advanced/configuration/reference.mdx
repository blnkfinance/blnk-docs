...
<Note>
If you set `BLNK_TRANSACTION_LOCK_DURATION` manually, run Blnk Core **0.15.1 or later**. On **0.10.3 through 0.15.0**, Go duration values such as `1m` could overflow to a ~147-year Redis lock TTL. The `1m` example in this reference is safe on 0.15.1+. [Lock settings →](/advanced/configuration/transactions#lock-settings) · [0.15.1 changelog →](/changelog/blnk-core)
</Note>
