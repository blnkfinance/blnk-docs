...
Under heavy contention, where many transactions compete for the same balance, throughput fell to 2,830 to 3,500 TPM. Even at the slowest rate measured, that is about **122 million transactions per month**. Across the contended workloads, monthly capacity ranged from **roughly 122 million to 151 million transactions**.

The API can accept traffic much faster than workers apply it, handling short bursts of around 260,000 TPM in these tests. Excess transactions are queued and applied asynchronously, and every accepted transaction in these runs eventually reached a balance.
...
These tests measure accepted throughput, applied throughput, and reliability in normal conditions and under balance contention. They do not measure [sharding](/guides/hot-balances#sharding-balances) or [hot-lane routing](/guides/hot-balances#hot-lane-routing).
