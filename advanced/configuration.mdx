...
<Tabs>
  <Tab title="Definitions">
    ...
    <Tip>The DNS string typically follows this pattern: `rediss://[[username:]password@]host[:port][/database]`</Tip>
    <Tip><b>Not recommended for production environments.</b> Set `skip_tls_verify` to true only if you understand the security implications and trust your network environment. Use this option primarily for development or when connecting to Redis instances with self-signed certificates.</Tip>
  </Tab>
</Tabs>
