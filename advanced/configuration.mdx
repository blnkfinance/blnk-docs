...
<Tabs>
  <Tab title="Definitions">
    ...
    | `insufficient_fund_retries` | boolean | Determines whether the ledger should automatically retry transactions that fail due to insufficient funds. |  |
    | `max_retry_attempts` | number | Specifies the maximum number of retry attempts before the transaction with insufficient funds is rejected. |  |
  </Tab>
</Tabs>
