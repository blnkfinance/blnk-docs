...
| Code | HTTP | Meaning |
| --- | --- | --- |
| `GEN_MALFORMED_REQUEST` | 400 | Request body could not be parsed (invalid JSON or wrong types) |
...
| `GEN_PAYLOAD_TOO_LARGE` | 413 | Reconciliation multipart upload exceeds the configured upload size. JSON bodies over the request size limit return `400 GEN_MALFORMED_REQUEST`. |
...
Errors when creating or fetching accounts. Codes use the `ACC_*` prefix.

| Code | HTTP | Meaning |
| --- | --- | --- |
| `ACC_NOT_FOUND` | 404 | Account does not exist |
| `ACC_DUPLICATE` | 409 | Account already exists |
| `ACC_GENERATION_FAILED` | 500 | Account number generation failed |
...
| Code | HTTP | Meaning |
| --- | --- | --- |
| `BAL_NOT_FOUND` | 404 | Balance does not exist, including a missing source or destination on a transaction route |
...
| `TXN_INVALID_AMOUNT` | 400 | `amount`, `precise_amount`, or `precision` must not be negative |
...
| `TXN_ALREADY_REFUNDED` | 409 | Transaction was already refunded |
...
| `TXN_VALIDATION_ERROR` | 400 | Transaction payload failed validation, including splitting sources and destinations at once |
...
| `RECON_UPLOAD_URL_INVALID` | 400 | Upload URL is missing a host or is otherwise invalid |
| `RECON_UPLOAD_HOST_NOT_ALLOWED` | 400 | Upload URL host is not in the allowed list |
...
Errors from queued async processing. Codes use the `QUEUE_*` prefix.

| Code | HTTP | Meaning |
| --- | --- | --- |
| `QUEUE_BACKPRESSURE` | 503 | Enqueue rejected because Redis memory or pending task count exceeded configured limits |
