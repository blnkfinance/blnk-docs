...
The master key is the `secret_key` from your configuration. Unlike [scoped API keys](/api-keys/overview), it is not bound to an owner or scopes, and it can call any endpoint on the server. Blnk also uses it to sign outbound [webhook](/advanced/notifications) and [transaction hook](/hooks/overview) deliveries.
...
3. **Manage hooks.** Register, update, list, and delete [transaction hooks](/hooks/create-hooks). Hook management rejects scoped keys, even with `hooks:*` scopes.

4. **Verify webhooks.** Blnk signs outbound deliveries with `X-Blnk-Signature` using `server.secret_key`. Use the same secret on your receiver to verify authenticity. See [Webhook security](/advanced/notifications#webhook-security-signature-verification).
