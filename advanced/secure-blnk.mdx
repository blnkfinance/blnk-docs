...
<Note>Secure mode authenticates callers. It does not encrypt traffic. For TLS, set `server.ssl` to `true`. See [Enable HTTPS](/advanced/enable-https).</Note>
...
- In production, enable HTTPS with `server.ssl`. See [Enable HTTPS](/advanced/enable-https)
</Step>
