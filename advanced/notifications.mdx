...
icon: "bell-ring"
description: "Configure real-time alerts for transaction events and handle error notifications through webhooks. Stay updated with critical information."
"og:title": "Webhooks • Blnk Developer Documentation"
"og:description": "Configure real-time alerts for transaction events and handle error notifications through webhooks."
---
...
Make handlers **idempotent**. Dedupe on a stable key such as `event` plus a resource identifier in `data` (for example `transaction_id` or `batch_id`), or on the combination of `X-Blnk-Signature` and the raw request body.

For Hooks, if your endpoint is unreachable or returns a non-2xx response, Blnk **retries delivery with backoff up until the max number of retries** instead of dropping the event. A delivery that failed while your receiver was down can arrive minutes later once the endpoint recovers.
