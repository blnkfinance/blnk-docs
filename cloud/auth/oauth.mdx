...
For third-party integrations, you'll need an access token to interact with the user's Cloud workspace via the [Cloud Proxy](/cloud/proxy/proxy-api), [Filters API](/cloud/proxy/filters-api), and [Data Lake API](/cloud/reference/data-lake-api).
...
curl -X POST "https://api.cloud.blnkfinance.com/transactions/filter?instance_id=YOUR_INSTANCE_ID" \
...
To route requests to the correct Blnk Core instance, include `instance_id` as a query parameter on every Proxy and Filters API request. Send it in the JSON body for Data Lake API requests.
...
<Card title="Filters API documentation" icon="list-filter" href="/cloud/proxy/filters-api">
Filter live ledger data.
</Card>

<Card title="Data lake API documentation" icon="water" href="/cloud/reference/data-lake-api">
