...
2. Blnk sends a `POST` request to your [portal generator URL](/cloud/apps/codebase-setup#set-up-app-routes).
...
| Issue | What to do |
| --- | --- |
| Password or 2FA is incorrect at launch | Check your password or 2FA code and try again. You may see `Authentication failed`. |
| Portal does not open | Confirm your `portal_generator_url` is reachable and returns a valid `portal_url` within about 10 seconds. |
| App opens but session expires quickly | Launch the app again from Cloud to get a fresh portal session. |
