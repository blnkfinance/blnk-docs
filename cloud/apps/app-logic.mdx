...
| API | When to use it |
| :-- | :-- |
| Filters API | Filter live ledger data through Cloud. |
...
For Proxy and Filters API requests, include the selected `instance_id` in the URL:
...
<Tab title="Using the Proxy API">
  Use the proxy when your app wants to create or update Core resources through Cloud. The format is:

  ```bash wrap
  https://api.cloud.blnkfinance.com/proxy/<registered-path>?instance_id=<instance_id>
  ```

  The path must be a registered Proxy route. Cloud does not forward every Core endpoint. You cannot list ledgers or fetch a balance, transaction, or identity by ID through the proxy. Use the Filters API for those reads.
...
<Tab title="Using the Filters API">
  Use the Filters API when your app needs to read live ledger data through Cloud.

  The format is:

  ```bash wrap
  https://api.cloud.blnkfinance.com/<resource>/filter?instance_id=<instance_id>
  ```

  `<resource>` is `ledger`, `balances`, `transactions`, `identities`, or `reconciliation`. Ledgers use the singular path `/ledger/filter`.

  For example, to filter transactions:

  ```bash wrap
  curl -X POST "https://api.cloud.blnkfinance.com/transactions/filter?instance_id=inst_..." \
    -H "Authorization: Bearer CLOUD_API_KEY" \
    -H "Content-Type: application/json" \
    -d '{
      "filters": [
  ...
  <Card title="Using the Filters API" icon="list-filter" href="/cloud/reference/filters-api">
    Filter endpoints and JSON filter bodies.
  </Card>
</Tab>
...
<Tab title="Other Cloud features">
  ...
  Alerts are a good example. You do not call alerts through `/proxy`, Filters, or the data lake. You call the Alerts API directly.
