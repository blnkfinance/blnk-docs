...
<Note>
Read our OAuth docs to [get the access token here](/cloud/reference/oauth#get-an-access-token).
</Note>
...
<Check>
If the transaction returns `status=QUEUED`, Core applies it asynchronously. To verify the final state, fetch the transaction (and updated balances) via the [Filters API](/cloud/proxy/filters-api).
</Check>
</Step>
</Steps>
...
<RelatedTopics
  items={[
    ...
    { title: "Filters API", href: "/cloud/proxy/filters-api" },
    { title: "Proxy API reference", href: "/cloud/reference/proxy-api" },
  ]}
/>
