...
<Steps>
  ...
  <Step title="Enable lineage tracking (optional)">
    ...
    - Select an `Allocation` strategy to control how tagged funds are used when the balance is debited: `FIFO` (first in, first out), `LIFO` (last in, first out), or `PROPORTIONAL` (spread debits across tagged funds).
