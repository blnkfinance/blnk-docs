...
After purchasing a Production License, you'll receive an email with a license file named `{license-name}.lic`.

You can activate your license in either of these ways:

1. **Use the license file directly:** Set `ENTERPRISE_LICENSE_FILE` to the license file path in step 4.
2. **Convert the file to base64:** Use the output as `ENTERPRISE_LICENSE_B64` in step 4.

To convert your license file to base64:

```bash
base64 -i ./licenses/{license-name}.lic | tr -d '\n'
```
</Step>
...
| `ENTERPRISE_LICENSE_B64` | Base64-encoded license string from Step 1. Takes precedence over `ENTERPRISE_LICENSE_FILE`. To use a file instead, set `ENTERPRISE_LICENSE_FILE` to the absolute path of your `.lic` file. |
</Step>
