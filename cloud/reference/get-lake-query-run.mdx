---
title: "Get query run"
api: "GET /data/lake/query-runs/{run_id}"
description: "Return one lake query run, including the SQL snapshot and a result preview."
---
...
This endpoint returns one run by `run_id`. You get the SQL and date range that actually ran, the status of that execution, and a preview of the result rows (at most 1000 rows, or less if the preview exceeds 2 MB). The SQL and dates are a snapshot from the time of the run.
...
<RequestExample>
```bash cURL wrap
curl -X GET 'https://api.cloud.blnkfinance.com/data/lake/query-runs/lake_query_run_c5d9e2a1-7b4f-4a3c-9e8d-1f6a2b4c8d30' \
  -H 'Authorization: Bearer CLOUD_API_KEY'
```
</RequestExample>

<ParamField path="run_id" type="string" required>
Unique id of the query run (`lake_query_run_...`). Get it from [Create new query](/cloud/reference/create-new-query) (`run_id`) or [List query runs](/cloud/reference/list-lake-query-runs).
</ParamField>
...
```json 200 wrap expandable
{
  "data": {
    ...
    "created_at": "2026-08-15T11:04:12.112739Z",
    "updated_at": "2026-08-15T11:04:12.954739Z",
    "rows": [
      {
        "currency": "USD",
        "txn_count": 1280
...
```json 404 wrap
{
  "error": {
    "code": "NOT_FOUND",
    "message": "lake query run not found"
  }
}
```
</ResponseExample>
