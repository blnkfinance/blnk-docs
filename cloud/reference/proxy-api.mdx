...
Bearer token for an API key or OAuth access token. Format: `Bearer YOUR_API_KEY` or `Bearer YOUR_ACCESS_TOKEN`. You can send `X-API-Key` or `X-blnk-key` instead.
...
Create credentials with Cloud proxy scopes in [API keys](/cloud/auth/api-keys) or [OAuth clients](/cloud/auth/oauth).
</ParamField>

<ParamField query="instance_id" type="string" required>
Unique identifier of the Blnk Core instance to target. Required on every Proxy API request. Get it from [list instances](/cloud/reference/list-instances) (`instance_...`), not from `deployment_id`.
</ParamField>
...
Cloud itself returns an error if `instance_id` is missing. `GET /proxy/ledgers/:id` is not a registered Proxy route and returns `404 page not found` as plain text.
...
<RelatedTopics
  items={[
    { title: "Proxy API guide", href: "/cloud/proxy/proxy-api" },
