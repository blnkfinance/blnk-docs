---
title: "List alert statuses"
api: "GET /alerts/statuses"
description: "List all custom alert statuses for your organization and the allowed colour codes."
---

import NeedHelp from "/cloud/snippets/need-help.mdx";
import Authorization from "/cloud/snippets/authorization.mdx";
import { RelatedTopics } from "/snippets/related-topics.jsx";

List all custom alert statuses for your organization and the allowed colour codes. Use this to build status pickers and show name/colour for each status.

---

<Authorization />

<RequestExample>
```bash cURL
curl -X GET 'https://api.cloud.blnkfinance.com/alerts/statuses' \
  -H 'X-blnk-key: CLOUD_API_KEY'
```
</RequestExample>

### Response

<ResponseExample>
```json 200
{
  "data": [
    {
      "organization_id": "org_01ABC123",
      "key": "PENDING_REVIEW",
      "name": "Pending Review",
      "colour": "#3B82F6",
      "created_at": "2025-01-29T14:00:00Z",
      "updated_at": "2025-01-29T14:00:00Z"
    }
  ],
  "allowed_colours": [
    "#3B82F6", "#10B981", "#F59E0B", "#EF4444", "#8B5CF6",
    "#EC4899", "#06B6D4", "#84CC16", "#F97316", "#6366F1",
    "#14B8A6", "#A855F7", "#E11D48", "#64748B", "#0EA5E9"
  ]
}
```
</ResponseExample>

<ResponseField name="data" type="array">
Array of custom status objects for the organization.
</ResponseField>

<ResponseField name="allowed_colours" type="array">
List of hex colour codes allowed when creating or updating a status.
</ResponseField>

<ResponseField name="organization_id" type="string">
Organization ID that owns the status.
</ResponseField>

<ResponseField name="key" type="string">
Unique key for the status (e.g. PENDING_REVIEW). Used when setting an alert's status and in update/delete URLs.
</ResponseField>

<ResponseField name="name" type="string">
Display name for the status.
</ResponseField>

<ResponseField name="colour" type="string">
Hex colour code for the status (one of allowed_colours).
</ResponseField>

<ResponseField name="created_at" type="timestamp">
ISO 8601 timestamp when the status was created.
</ResponseField>

<ResponseField name="updated_at" type="timestamp">
ISO 8601 timestamp when the status was last updated.
</ResponseField>

---

<NeedHelp />

<RelatedTopics
  items={[
    { title: "Alert statuses", href: "/cloud/alerts/alert-statuses" },
    { title: "Create alert status", href: "/cloud/reference/create-alert-status" },
    { title: "Delete alert status", href: "/cloud/reference/delete-alert-status" },
  ]}
/>
