...
icon: "book-open"
description: "Browse Blnk Cloud REST endpoints for OAuth authentication, Deployments, Billing, Audit, Proxy, Data lake, Filters, Alerts, and MCP integrations."
og:title: "Cloud API Reference Overview | Blnk Cloud Guide"
og:description: "Browse Blnk Cloud REST endpoints for OAuth authentication, Deployments, Billing, Audit, Proxy, Data lake, Filters, Alerts, and MCP integrations."
---
...
7. **Deploy managed Core instances** without using the dashboard
8. **Read billing and usage** without using the dashboard
9. **Query the data lake** with read-only SQL
10. **Read audit logs** without using the dashboard
...
5. If it's a **filter request** (`POST /{resource}/filter`):
    - Blnk Cloud queries the instance database directly using the JSON filter body and returns the results.
6. If it's a **data lake request** (`/data/lake/*`):
    - Blnk Cloud runs read-only SQL against the instance data lake and returns the rows.
7. If it's a **deployments request** (`/deployments/*`):
    - Blnk Cloud creates or manages a managed Core instance in your workspace and returns the deployment object.
    - `GET /instances/{instance_id}` returns full details for a connected instance. Use that `instance_id` on Proxy, Filters, and Data Lake.
8. If it's a **billing request** (`/billing/*`):
    - Blnk Cloud returns plan, invoice, or usage for your workspace.
9. If it's an **audit request** (`/audit/*`):
    - Blnk Cloud returns activity rows for your workspace.
10. For other direct requests (alerts, MCP), Blnk Cloud processes the request and returns the response to your app.
...
<CardGroup cols={2}>
  <Card title="Deployments API" icon="cloud-upload" href="/cloud/reference/create-deployment">
    Create and manage managed Core instances.
  </Card>
  <Card title="Billing API" icon="credit-card" href="/cloud/reference/get-billing-status">
    Read plans, invoices, and usage.
  </Card>
  <Card title="Audit API" icon="scroll" href="/cloud/reference/list-audit-logs">
    Read workspace activity, including actions performed by API keys.
  </Card>
  <Card title="Proxy API" icon="arrow-left-right" href="/cloud/reference/proxy-api">
    Create and update Core resources.
  </Card>
  <Card title="Data lake API" icon="water" href="/cloud/reference/data-lake-api">
    Run read-only SQL against an instance data lake.
  </Card>
