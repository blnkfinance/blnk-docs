...
icon: "book-open"
description: "Browse Blnk Cloud REST endpoints for OAuth authentication, Deployments, Billing, Audit, Proxy, Data, Data lake, Filters, Alerts, and MCP integrations."
og:title: "Cloud API Reference Overview | Blnk Cloud Guide"
og:description: "Browse Blnk Cloud REST endpoints for OAuth authentication, Deployments, Billing, Audit, Proxy, Data, Data lake, Filters, Alerts, and MCP integrations."
---
...
7. If it's a **filter request** (`POST /{resource}/filter`):
    - Blnk Cloud queries the instance database directly using the JSON filter body and returns the results.
8. If it's a **deployments request** (`/deployments/*`):
    - Blnk Cloud creates or manages a managed Core instance in your workspace and returns the deployment object.
    - `GET /instances` lists connected instances and is the source of `instance_id` for Proxy and Data.
9. If it's a **billing request** (`/billing/*`):
    - Blnk Cloud returns plan, invoice, usage, or a Stripe checkout or portal URL for your workspace.
10. If it's an **audit request** (`/audit/*`):
    - Blnk Cloud returns activity rows for your workspace.
11. For other direct requests (alerts, MCP), Blnk Cloud processes the request and returns the response to your app.
...
<CardGroup cols={2}>
  ...
  <Card title="Billing API" icon="credit-card" href="/cloud/reference/get-billing-status">
    Read plans, invoices, usage, and Stripe checkout or portal URLs.
  </Card>
