...
<Tabs>
  ...
  <Tab title="balances">
    ...
    | `balance` | Balance amount in smallest units as copied that day (no `precision` column on balances) | Inspect a single day copy. Not a live holding. Do not `SUM` across days or currencies. For current amounts, use the [Ledger API](/reference/get-balance). |
