...
Use this when you need a count of wallets per currency, plus total balance. It helps you see how your portfolio is split across currencies.

```sql
SELECT
  currency,
  COUNT(*) AS balance_count,
  SUM(TRY_CAST(balance AS DECIMAL(38, 8))) AS total_balance
FROM balances
GROUP BY currency
