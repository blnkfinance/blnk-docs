...
Use this when you need how many distinct wallets appear in the selected date range, by currency. This is not a live portfolio total. `created_at` on balances is wallet creation time, so you cannot safely pick a latest balance snapshot from the lake. For current holdings, use the [Filters API](/cloud/reference/filters-api). See [How the lake works](/cloud/insights/lake).
