...
<ParamField body="sources" type="array" required>
  ...
  <Expandable>
    ...
    <ParamField body="precise_distribution" type="string">
      Specifies a fixed precise amount to send from this source when using `precise_amount`. Provide the amount in its smallest unit (e.g., `"37841507139655800"`).
      
      <Note>
      Use `precise_distribution` only for precise amounts when using `precise_amount`. For float amount, percentage, and "left", use `distribution` instead. You can mix both fields in the same transaction.
      </Note>
    </ParamField>
  </Expandable>
</ParamField>
