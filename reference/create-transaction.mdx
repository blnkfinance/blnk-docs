...
<ParamField body="skip_queue" type="boolean">
  When `true`, it bypasses the default transaction queuing system and processes transactions directly. If the source has insufficient funds, Blnk returns `400 TXN_INSUFFICIENT_FUNDS` and does not use the `reference`, so you can retry the same `reference`. Confirmed on Blnk Core 0.15.0. See [Managing insufficient funds](/transactions/introduction#managing-insufficient-funds). See also: [Skip transaction queue](/transactions/introduction#skip-transaction-queue).
</ParamField>
...
```json 400 Insufficient Funds with Skip Queue
{
