...
<ParamField body="ledger_id" type="string" required>
  Unique id of the ledger the balance is created in. Use `general_ledger_id` for an [internal balance](/balances/internal-balances).
</ParamField>
...
<ParamField body="indicator" type="string">
  General Ledger indicator for an [internal balance](/balances/internal-balances). Must start with `@` and contain no whitespace. Only allowed when `ledger_id` is `general_ledger_id`. 
  
  Available in version 0.15.3 and later. The same indicator and currency pair can only exist once.
</ParamField>
...
<ResponseField name="indicator" type="string">
  The balance indicator. Set for General Ledger balances created with `indicator`.
</ResponseField>
...
```json 400
{
  "errors": "ledger_id: cannot be blank.",
  "error_detail": {
