...
<RequestExample>
```json
curl -X GET "http://localhost:5001/transactions/reference/{reference}" \
...
```json 404
{
  "error": "Transaction with reference 'ref_does_not_exist' not found",
  "error_detail": {
    "code": "TXN_NOT_FOUND",
    "message": "Transaction with reference 'ref_does_not_exist' not found"
  }
}
```
</ResponseExample>
