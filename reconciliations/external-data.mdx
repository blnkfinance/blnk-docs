...
<CodeGroup>
```bash cURL wrap
curl -X POST "https://YOUR_BLNK_INSTANCE_URL/reconciliation/upload" \
...
```go Go wrap
file, _ := os.Open("transactions.json")
result, resp, err := client.Reconciliation.Upload("Stripe", file, file.Name())
```

```python Python wrap
response = blnk.reconciliation.upload("transactions.json", "Stripe")
```
