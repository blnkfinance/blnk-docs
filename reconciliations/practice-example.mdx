...
<Card title="External data source" icon="file-spreadsheet" href="https://github.com/blnkfinance/blnk-docs/blob/main/resources/recon-example-external-source.csv">
  Sample CSV for reconciliation practice.
</Card>
...
const response = await blnk.Reconciliation.upload(
  'recon-example-external-source.csv',
  'Stripe',
);
```

```go Go wrap
file, _ := os.Open("recon-example-external-source.csv")
result, resp, err := client.Reconciliation.Upload("Stripe", file, file.Name())
```

```python Python wrap
response = blnk.reconciliation.upload("recon-example-external-source.csv", "Stripe")
```
...
<CodeGroup>
```bash cURL wrap expandable
curl -X POST "http://localhost:5001/reconciliation/matching-rules" \
  -H "X-blnk-key: <api-key>" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "Basic matcher",
...
<CodeGroup>
```bash cURL wrap
curl -X POST "http://localhost:5001/reconciliation/start" \
  -H "X-blnk-key: <api-key>" \
  -H "Content-Type: application/json" \
  -d '{
    "upload_id": "upload_0cd4a87d-544c-4b7e-9232-f36e79631c32",
...
result, resp, err := client.Reconciliation.Run(blnkgo.RunReconData{
    UploadID: "upload_0cd4a87d-544c-4b7e-9232-f36e79631c32",
...
<CodeGroup>
```bash cURL wrap
curl -X GET "https://YOUR_BLNK_INSTANCE_URL/reconciliation/{reconciliation_id}" \
  -H "X-blnk-key: <api-key>"
```
