...
curl -X POST "http://YOUR_BLNK_INSTANCE_URL/reconciliation/start" \
  ...
  -d '{
    ...
    "matching_rule_ids": [
      ...
      "rule_a1b2c3d4-5678-90ef-abcd-ef1234567890"
    ]
  }'
```
...
<CodeGroup>
```bash cURL wrap
curl -X GET "http://YOUR_BLNK_INSTANCE_URL/reconciliation/recon_6e6feddd-930b-4e3a-8ba1-a3eee659bb3c" \
  -H "X-blnk-key: <api-key>"
```

```typescript TypeScript wrap
const response = await blnk.Reconciliation.get(
  'recon_6e6feddd-930b-4e3a-8ba1-a3eee659bb3c',
);
```
...
```java Java wrap
ApiResponse<JsonNode> response = blnk.reconciliation().get(
    "recon_6e6feddd-930b-4e3a-8ba1-a3eee659bb3c"
);
```
...
```python Python wrap
response = blnk.transactions.get("{transaction_id}")
```
...
```json Response {5} wrap
{
  ...
  "meta_data": {
    ...
    "reconciliation_id": "recon_9134cb8a-a3f6-47e9-b566-4fb93fa051e7"
  }
}
```
