...
For **many-to-one transactions**, you can include a key or value to the description of the transactions to make it easy to identify similar transactions and group them together as one. E.g., if a user makes a transfer, you can pass `bankTransfer-209018` in the description so that all external records with the description can be grouped together to be matched to an internal record. 
