...
<Steps>
  ...
  <Step title="Create a 'docker-compose.yml' file">
    ...
    <Note>
      The default image tag is `jerryenebeli/blnk:0.14.5`. Override it with the `BLNK_IMAGE` environment variable when you deploy — for example `BLNK_IMAGE=jerryenebeli/blnk:latest docker compose up -d`.
    </Note>
