...
<Check>
Congratulations! You've successfully created your first transaction in Blnk. The transaction is now queued and will be processed automatically.
</Check>

Finally, let's confirm that the destination balance has been funded:

```bash cURL
curl -X GET 'http://localhost:5001/balances/indicator/MyBalance/currency/USD'
```
...
Now that you've created your first transaction, explore these key concepts to build more sophisticated financial applications:

<CardGroup cols={2}>
  <Card title="Ledgers" icon="book" href="/ledgers/introduction">
    Learn how to organize your financial data with ledgers and understand the General Ledger.
  </Card>
  
  <Card title="Balances" icon="wallet" href="/balances/introduction">
    Create and manage balances to represent wallets, accounts, and other stores of value.
  </Card>
  
  <Card title="Transactions" icon="messages-dollar" href="/transactions/introduction">
    Master transaction recording, statuses, and advanced features like inflight transactions.
  </Card>
  
  <Card title="Reconciliations" icon="object-union" href="/reconciliations/overview">
    Ensure data accuracy by reconciling your Blnk data with external sources.
  </Card>
  
  <Card title="Identities" icon="user" href="/identities/introduction">
    Secure your data with PII tokenization and identity management.
  </Card>
  
  <Card title="Hooks" icon="link" href="/hooks/overview">
    Automate workflows and integrate with external systems using webhooks.
  </Card>
</CardGroup>
