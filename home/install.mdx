---
title: "Getting Started with Blnk Core"
...
Manually install Blnk on your own infrastructure. This option requires [Docker](https://www.docker.com/) and [Compose](https://docs.docker.com/compose/).
...
On first run, Docker Compose starts an `init` service that creates `blnk.json` with compose defaults when the file is missing. Edit `blnk.json` to customize your configuration before restarting if needed.

<Check>
Your Blnk server is now running! You can verify it's working by visiting `http://localhost:5001`.
</Check>
...
<Note>
To customize your Blnk configuration, see [Configuration overview](/advanced/configuration/overview).
</Note>
...
<iframe
  ...
  src="https://www.youtube.com/embed/zFVorhVUv6k"
...
<CardGroup cols={2}>
  ...
  {/* <Card title="Reconciliations" icon="copy" href="/reconciliations/overview">
    Match ledger data with external sources.
  </Card> */}
  ...
  {/* <Card title="Webhooks" icon="link" href="/webhooks/overview">
    React to Core events with webhooks.
  </Card> */}
</CardGroup>

<Note>
Using an SDK? See the [TypeScript](/sdks/typescript), [Go](/sdks/go), [Python](/sdks/python), or [Java](/sdks/java) quick starts.
</Note>
