...
"og:description": "Learn how identities work in Blnk."
---
...
<Tabs>
<Tab title="Create a new individual">
...
<Tab title="Create a new organization">
To create a new organization, call the **Create Identity** endpoint:
...
| Property | Description | Type |
| :--- | :--- | :--- |
| `identity_type` | Specifies the identity type as "organization". | `string` |
| `organization_name` | Name of the organization | `string` |
| `email_address` | Email ID of the address | `string` |
| `phone_number` | Contact number of the individual | `string` |
| `category` | Type of organization based on your application, e.g. bank, retail, etc. | `string`
| `street` | Street address of the identity. | `string` |
| `country` | Residential country of the identity. | `string` |
| `state` | State of residence of the identity. | `string` |
| `post_code` | Postal code related to the identity. | `string` |
...
- **Unified records:** Storing identities on a ledger provides a single, reliable source of truth for all customer data. This ensures that every team—technical or non-technical—has consistent access to the same information.
...
- **Secure infrastructure:** Blnk employs enterprise-grade security controls to safeguard customer data against unauthorized access. Our commitment to security and compliance is demonstrated through our certifications and adherence to industry standards, including ISO 27001, PCI-DSS, SOC 2, and GDPR. These measures not only protect your data but also reinforce trust with your customers.

### 3. Data-driven customer insights

- **Behavioural patterns:** By linking identities to transactions, you can get & use insights into the spending habits, unusual activity, and long-term trends. These insights help refine strategies and improve customer offerings.
