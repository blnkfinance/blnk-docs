...
ApiResponse<JsonNode> response = blnk.identity().create(
    IdentityData.create()
        ...
        .metaData(Map.ofEntries(
            Map.entry("customer_id", "CUST123456"),
            Map.entry("membership_level", "Gold"),
            Map.entry("preferred_language", "English"),
            Map.entry("verification_data", Map.of(
                "verified_by", "ThirdPartyService",
                "verification_date", "2024-07-01T10:00:00Z",
                "verification_status", "verified"
            ))
...
```go Go wrap
identity, resp, err := client.Identity.Create(blnkgo.Identity{
    IdentityID: "idt_8cc22560-5ace-4989-8953-fcbad7947d48",
    IdentityType: blnkgo.Individual,
    FirstName: "Alice",
    LastName: "Smith",
    EmailAddress: "alice.smith@example.com",
...
You can view all identities or retrieve a specific identity by its ID:

<CodeGroup>
```bash cURL (all identities) wrap
curl -X GET "http://YOUR_BLNK_INSTANCE_URL/identities" \
  -H "X-blnk-key: <api-key>"
```

```bash cURL (single identity) wrap
curl -X GET "http://YOUR_BLNK_INSTANCE_URL/identities/{identity_id}" \
  -H "X-blnk-key: <api-key>"
```

```bash cURL (with filters) wrap
# View all identities from Canada in the "customer" category
curl -X GET "http://YOUR_BLNK_INSTANCE_URL/identities?country_eq=Canada&category_eq=customer" \
  -H "X-blnk-key: <api-key>"
```
</CodeGroup>

<Tip>
To learn about filtering directly with operators like `eq`, `ilike`, `in`, and more, see [Advanced Filtering](/search/db/filtering).
</Tip>
*/}
...
```python Python wrap expandable
response = blnk.identity.update("{identity_id}", {
  "identity_type": "individual",
  "first_name": "Alice",
  "last_name": "Smith",
  "other_names": "Marie",
...
```python Python wrap
response = blnk.identity.delete(
  "idt_8cc22560-5ace-4989-8953-fcbad7947d48",
)
```

```java Java wrap
ApiResponse<JsonNode> response = blnk.identity().delete(
    "idt_8cc22560-5ace-4989-8953-fcbad7947d48");
```
