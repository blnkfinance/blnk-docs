...
<Steps titleSize="h3">
<Step id="prepare-historical-data" title="Prepare your historical data">
...
<Step id="choose-migration-approach" title="Choose your migration approach">
...
<Step id="execute-migration" title="Execute the migration">
...
<Steps titleSize="h3">
<Step id="creating-bulk-backdated-transactions" title="Creating bulk backdated transactions">
...
```go Go wrap
effectiveDate1, _ := time.Parse(time.RFC3339, "2024-09-15T10:30:00Z")
effectiveDate2, _ := time.Parse(time.RFC3339, "2024-09-18T14:15:00Z")
...
ApiResponse<JsonNode> response = blnk.transactions().createBulk(
    BulkTransactions.create()
        ...
        .transactions(List.of(
            ...
            .source("@source_account")
            .destination("@destination_account")
            .effectiveDate("2024-09-15T10:30:00Z"),
            CreateTransactions.create()
            .preciseAmount(125000)
            .precision(100)
            .reference("migration_2024_002")
            ...
            .source("@source_account")
            .destination("@vendor_account")
            .effectiveDate("2024-09-18T14:15:00Z")
        )));
```
</CodeGroup>
...
<Tip>Recommended batch size is 1,000–5,000 transactions. Max is 10,000 transactions per bulk request; split larger migrations across multiple calls.</Tip>

</Step>
<Step id="bulk-transaction-webhook" title="Webhook notification upon successful completion">
...
2. **Balance validation:**
    ...
    After each migration phase, verify account balances against your source system to ensure accuracy. Use the [Historical Balance API](/balances/historical-balances) to check balances at specific points in time:

    <CodeGroup>
    ```bash cURL wrap
    curl -X GET "http://YOUR_BLNK_INSTANCE_URL/balances/{balance_id}/at?timestamp=2024-12-31T23:59:59Z" \
      -H "X-blnk-key: <api-key>"
    ```
    ...
    ```go Go wrap
    at, _ := time.Parse(time.RFC3339, "2024-12-31T23:59:59Z")
    balance, resp, err := client.LedgerBalance.GetHistorical("{balance_id}", at, false)
    ```
