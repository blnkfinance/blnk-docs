...
<Steps>
    <Step title="Create a new ledger">
        ...
        <CodeGroup>
        ```bash cURL wrap
        curl -X POST "http://YOUR_BLNK_INSTANCE_URL/ledgers" \
        ...
        const response = await blnk.Ledgers.create({
          name: 'User balances',
          meta_data: { project_owner: 'Blnk Finance' },
        });
        ```
        ...
        ApiResponse<JsonNode> response = blnk.ledgers().create(
            CreateLedger.create()
                .name("User balances")
                .metaData(Map.of("project_owner", "Blnk Finance")));
        ```
        </CodeGroup>
    ...
    <Step title="Create a balance for User A in the ledger you've just created">
        ...
        <CodeGroup>
        ```bash cURL wrap
        curl -X POST "http://YOUR_BLNK_INSTANCE_URL/balances" \
          -H "X-blnk-key: <api-key>" \
          -H "Content-Type: application/json" \
          -d '{
            "ledger_id": "ldg_073f7ffe-9dfd-42ce-aa50-d1dca1788adc",
    ...
    <Step title="Record the transaction between the user and your company">
        ...
        transaction, resp, err := client.Transaction.Create(blnkgo.CreateTransactionRequest{
          ParentTransaction: blnkgo.ParentTransaction{
            PreciseAmount: big.NewInt(300000),
            Reference:   "ref_8728781718",
            Currency:    "USD",
            Precision:   100,
            Source:      "bln_ebcd230f-6265-4d4a-a4ca-45974c47f746",
        ...
        ApiResponse<JsonNode> response = blnk.transactions().create(
            CreateTransactions.create()
                .reference("ref_8728781718")
                .preciseAmount(300000)
                .currency("USD")
                .precision(100)
                .source("bln_ebcd230f-6265-4d4a-a4ca-45974c47f746")
                .destination("@CompanyRevenue")
                .description("Invoice payment")
                .metaData(Map.of("user_id", "user_a")));
        ```
        </CodeGroup>

        ```json Response
        {
          "transaction_id": "txn_171821872187",
          "reference": "ref_8728781718",
          "amount": 3000,
          "precise_amount": 300000,
          "currency": "USD",
          ...
          "destination": "@CompanyRevenue",
          "status": "QUEUED",
          "created_at": "2024-04-23T12:32:00Z"
        }
        ```
