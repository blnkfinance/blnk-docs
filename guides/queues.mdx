...
description: "Ensuring fairness, high-traffic protection, and consistency in Blnk."
icon: "filter-list"
"og:title": "Understanding Queues • Blnk Developer Documentation"
"og:description": "Ensuring fairness, high-traffic protection, and consistency in Blnk."
---
...
When building financial systems, the choice between immediate and queue-based transaction processing directly affects reliability and performance. 
...
<Accordion title="Queue vs Skip Queue Flow Diagram">
...
flowchart TD
    ...
    J[Acquire Lock]
    K{Inflight?}
    L["Mark Transaction as INFLIGHT<br/>(No balance update yet)"]
    M[Client Commits/Voids]
    O[Send Webhook Event]
    Q[Apply Transaction, Update Balance]
    S[Record next state: applied, inflight, or rejected]
...
When a queued transaction moves to its next state (`INFLIGHT`, `APPLIED`, or `REJECTED`), Blnk creates a new record that maintains a connection to the original queued transaction. This connection is maintained in two ways:
...
**Instead of:** `Customer balance → Stripe`

**Use:** `Customer balance → Customer hold balance → Stripe`
...
1. **Customer balance → Customer hold balance** (can use `skip_queue: true` for immediate processing)
   ...
   * It acts like a mini balance that represents the customer's interaction with Stripe, but scoped only to that customer to avoid lock contention
   * Each customer gets their own hold balance, eliminating competition for locks

2. **Customer hold balance → Stripe** (use queue, don't skip)
   * Internally, funds can be bulk settled into Stripe using the queue system
   * This allows for efficient batch processing and reduces the load on the Stripe balance

This moves pressure away from the Stripe balance and provides you with more flexibility in processing high-traffic transactions.
