...
"og:description": "Learn to create hooks in Blnk. Step-by-step guide for registering webhooks, handling events, and integrating real-time notifications"
---
...
```
<CodeGroup>
```bash Create pre-transaction hook
...
```
```bash Create post-transaction hook
curl -X POST "http://YOUR_BLNK_INSTANCE_URL/hooks" \
  -H "X-Blnk-Key: YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
