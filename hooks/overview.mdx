...
<Note>
Blnk signs hook requests with `X-Blnk-Signature` and `X-Blnk-Timestamp` headers. See [Webhook security](/advanced/notifications#webhook-security-signature-verification) to learn how to verify hook authenticity.
</Note>

<Warning>
Starting in **0.14.3**, creating, updating, viewing, listing, and deleting hooks requires the master key (`server.secret_key`). Regular API keys—including those with `hooks` scopes—receive `403 Forbidden` on hook management endpoints.
</Warning>
