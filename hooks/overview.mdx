...
<Note>
When running in secure mode, Blnk signs hook requests with `X-Blnk-Signature` and `X-Blnk-Timestamp` headers. See [Webhook security](/advanced/notifications#webhook-security-signature-verification) to learn how to verify hook authenticity.
</Note>
