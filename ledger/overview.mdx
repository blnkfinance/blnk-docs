...
Building fintech requires foundational knowledge of digital money movement and storage to accurately keep and manage transaction records. As part of the package, many moving parts — banking and payment providers, AML policies, settlement and reconciliation, etc. — also depend on how well your transactions are recorded.

It becomes crucial to you, as a developer, and your organization to have a ledger that scales as your internal financial operations/capabilities scale. Because transaction records classify as very sensitive data, it is your responsibility to build and maintain an accurate, reliable, scaleable, and secure database that everything else in your fintech application seamlessly connects to.

We understand that this is hard without deep industry (or financial accounting) experience. 

Hence, the Blnk Ledger.

### Why bother?

All fintech is built on ledgers. Why? Because fintech deals with transaction activities, and these activities have to be stored, tracked, and quantified in real-time. Whether it is banking, payments, lending, or embedded finance, at any given time, funds are either being sent, accepted or stored. 

How an organization records these activities reflects their commitment to customer safety and security. At Blnk, we use 3 metrics to determine this commitment (and we designed the Blnk Ledger with them in mind):

1. **It is accurate:** All records must correctly reflect the right information per transaction to the smallest unit. This makes it very easy to perform settlements and reconciliation tasks on any account (internal or external), ensuring that you are always in compliance with your local regulatory financial laws.

2. **It can scale:** Whether it is a few hundred or millions of transactions per second, you can trust your ledger to manage the traffic excellently. As your organization grows and becomes more capable to handle advanced financial operations, a great ledger scales with you offering you flexible control on your financial data.
...
In the Blnk Ledger, **Balances** are like files that contain all the credit and debit transactions that's happened on a balance. In any fintech, these files are always growing — tens, hundreds, or thousands getting added every day depending on how you record your transactions. The more files you have, the higher the risk of chaos in your system.

To manage or remove this risk, you need a file manager. Traditional file managers allow you to use folders to group similar files together for easy navigation or reference. In Blnk, we use **Ledgers** as folders to manage and store your **Balances**.
...
The Blnk Ledger gives you full control and flexibility when building your fintech application; however, you can only get maximum value from the Blnk Ledger if you critically think about your ledger architecture.
...
Unlike traditional file managers, the folders must exist first before the files can be created. Most importantly, their location cannot be changed once created. 
