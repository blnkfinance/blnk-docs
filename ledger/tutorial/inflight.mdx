...
Inflight, `inflight`, is a transaction parameter that instructs Blnk, in a payout workflow, to hold on applying a transaction until it has actually happened in real life.
...
#### After real-life processing

Once the transaction has been processed in the real world, Blnk expects one of two instructions — commit or void.
...
<Steps>
	...
	<Step title="Instruct Blnk what to do next">
		...
		|`inflight_status` | Status of the transaction in real life. `commit` means it was successful and the transaction should be applied in your ledger. `void` means it failed and the transaction should be rejected. | Yes | String
