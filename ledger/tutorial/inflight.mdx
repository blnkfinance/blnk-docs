...
<Steps>
	<Step title="Initiate a transaction">
		Send a request to the [Record Transaction](record-a-transaction) endpoint and input the required fields. Include `"inflight": true` to enable Inflight for the transaction.
	...
	<Step title="Transaction is added to queue for processing">
		The transaction is immediately [added to a queue](../guide/queuing-transactions) for processing with its status initially set to `QUEUED`. 
