...
<Steps>
	...
	<Step title="Instruct Blnk what to do next">
		Once the condition is met, call the [Inflight](/misc/update-inflight-transaction) endpoint URL. Provide the `id` of the transaction and the real-life status of the transaction. 
		...
		|`inflight_status` | Instruction to Blnk depending on if the condition has been met. Send `commit` if it was successful and the transaction will be applied in your ledger OR `void` if it failed and the transaction should be rejected. | Yes | String
...
There are three total inflight statuses. Please note, an inflight status is different from a transaction status. An `inflight_status` field represents the instruction given to Blnk to apply to the transaction.

- `commit`: This means the inflight conditions have been met and the transaction should be applied to its balances.

- `void`: This means the inflight conditions were not met and the transaction should not be applied to its balances.

- `expired`: This means the transaction stayed in the `INFLIGHT` status past the specified expiry date. The `inflight_status` is saved as "expired" and the transaction is discarded.
