...
You can provide more than one query text in the `q` field, e.g., `"q": "usd fees inflight"` will return documents that contain at least one of these words in their results.

If you want to explicitly exclude a word in your query, you can prefix the word with the "–" operator, e.g. `"q": "usd fees -inflight"` will return documents that contain `usd` or `fees`, but exclude all documents that contain `inflight`.
