...
Balances are always updated whenever a new transaction is recorded, and they can be used to track the flow of funds within the organization ensuring that your books are always balanced.

Every **Balance** is made up of 3 sub-balances:

1. **Credit balance:** The total sum of all transactions where the balance was `Source`. 
2. **Debit balance:** The total sum of all transactions where the balance was `Destination`. 
...
You can request to see a list of all balances in your system, or the details of a particular balance.
...
- Call the same URL without the `balance_id` to see a list of all the balances in your system.
