...
```
```Go Golang
newTransaction, _ := newBlnk.Transactions.RecordTransaction(context.Background(), transactions.RecordTransactionRequest{
  Amount:         blnk.Int64(1000),
  Reference:      blnk.String("ref-0-1"),
  Currency:       blnk.String("USD"),
  Source:         blnk.String("@world"),
  Destination:    newBalance.BalanceId,
  AllowOverDraft: blnk.Bool(true),
})
```
</CodeGroup>
