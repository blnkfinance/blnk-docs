...
**See also → [Managing multiple sources](multiple-sources).**
...
<Steps>
	...
	<Step title="Initiate the transaction">
		...
		<Note>You can also include a `meta_data` object (optional) in your request when receiving money into multiple destinations. </Note>
	...
	<Step title="Send your request">
		Once your request is received, Blnk automatically creates separate transaction records for each destination with their respective amounts. This is crucial because Blnk runs on the [Double Entry Accounting Principle](../guide/double-entry-101).
...
<CardGroup>
	<Card title="Managing multiple sources" icon="book-open-cover" href="multiple-sources">
