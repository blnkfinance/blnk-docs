...
<Steps>
	...
	<Step title="Initiate the transaction">
		...
		| `precision` | Converts the transaction amount to the lowest unit possible. **See also → [Understanding precision](/ledger/tutorial/precision).** | No | `int64`
		...
		<Note>You can include also a `meta_data` object (optional) in your request when sending money from multiple sources. </Note>
