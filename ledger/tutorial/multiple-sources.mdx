...
'og:title': 'Multiple sources - Blnk'
'og:description': 'Learn how to transfer money from multiple sources to a single destinations with Blnk.'
...
**See also → [Managing multiple destinations](multiple-destinations).**
...
<Steps>
	...
	<Step title="Initiate the transaction">
		...
		<Note>You can also include a `meta_data` object (optional) in your request when sending money from multiple sources. </Note>
		
		<Note>When sending from multiple sources, you do not need to include the `source` field in your payload. Use the `sources` array to group the participating balances in your payload.</Note>
	</Step>
	<Step title="Send your request">
		Once your request is received, Blnk automatically creates separate transaction records for each source with their respective amounts. This is crucial because Blnk runs on the [Double Entry Accounting Principle](../guide/double-entry-101).
		...
		Transactions are queued by default waiting to be [applied](record-a-transaction#transaction-statuses). We will notify you via the `transaction.applied` webhook once they are done.
	</Step>
</Steps>
...
**See also → [Search your Blnk data](search/overview).**

## See also

<CardGroup>
	<Card title="Managing multiple destinations" icon="book-open-cover" href="multiple-destinations">
