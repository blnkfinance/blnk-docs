...
<Steps>
	<Step title="Determine and set your conditions">
		...
		Call the [Create Balance Monitor](/misc/create-balance-monitor) endpoint and pass the following attributes in the request payload:
		...
		- `balance_id`: Unique identifier of the balance to be monitored **(string)**.
		- `condition`: Object representing the condition to be satisfied.
		- `field`: Specific sub-balance to monitor **(string)**. It can be `debit_balance`, `credit_balance`, or `balance`.
		- `operator`: Indicates the comparison operation to be performed between the field and operator **(string).** **See below → [Supported operators](#supported-operators).**
	...
	<Step title="You're good to go!">
		Blnk stores your monitoring request with a unique `monitor_id`. Once the condition is met, you will get instantly notified via the `balance.monitor` webhook. 
		...
		**See also → [Setting up notifications](/ledger/guide/notifications).**
		...
		- `description`: Description of your balance monitor **(string)**. It is left empty if it's not passed in the request.
