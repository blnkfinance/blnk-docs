...
**See also → [Create a balance](create-a-balance).**
...
<Steps>
	<Step title="Determine and set your conditions">
		...
		<Note>You can also include a `meta_data` in your request if you need to add custom data to your balance monitor. </Note>
	</Step>
	<Step title="You're good to go!">
		...
		**See also → [Setting up notifications](/intro/notifications).**
...
To update the conditions of a particular balance monitor, call the [Update balance monitor](/misc/update-balance-monitor) endpoint using the `monitor_id` and pass the updated conditions.
