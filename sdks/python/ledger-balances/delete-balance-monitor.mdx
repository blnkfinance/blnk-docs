---
mode: "wide"
title: "Delete balance monitor"
sidebarTitle: "Delete balance monitor"
api: "DELETE /balance-monitors/{monitor_id}"
description: "Delete a balance monitor with the Python SDK."
---

import NeedHelp from "/snippets/need-help.mdx";
import { RelatedTopics } from "/snippets/related-topics.jsx";

Use `blnk.balance_monitor.delete` to remove a [balance monitor](/balances/balance-monitoring) by its ID.

<Steps>
<Step title="Call the method">

```python blnk.balance_monitor.delete wrap
response = blnk.balance_monitor.delete("mon_e0e77b0c-4985-472a-9bf5-76a48b0259b0")
```

| Field | Type | Description |
| :-- | :-- | :-- |
| `id` | `str` | Monitor ID to delete. |

</Step>

<Step title="Confirm the outcome">

Check the response message to confirm the monitor was removed. Blnk will no longer check thresholds or send webhooks for that monitor.

</Step>

<Step title="Response">

```json 200 OK wrap
{
  "message": "BalanceMonitor deleted successfully"
}
```

| Field | Type | Description |
| :-- | :-- | :-- |
| `message` | `str` | Confirmation that the monitor was deleted. |

</Step>
</Steps>

---

<NeedHelp />

<RelatedTopics
  items={[
    { title: "Balance monitoring", href: "/balances/balance-monitoring" },
    { title: "Global webhooks", href: "/webhooks/global-webhooks" },
    { title: "Balances overview", href: "/balances/introduction" },
    { title: "Delete Balance Monitor", href: "/reference/delete-balance-monitor" },
  ]}
/>
