...
The SDK exports these codes as `BlnkErrorCode`:

| Constant | When Core returns it |
| :-- | :-- |
| `BlnkErrorCode.TXN_VALIDATION_ERROR` | Negative `amount` or `precision`, or source equal to destination |
| `BlnkErrorCode.GEN_CONFLICT` | Duplicate General Ledger indicator and currency (`409`) |
| `BlnkErrorCode.TXN_INVALID_AMOUNT` | Invalid amount (see the [API error codes](/advanced/error-codes) catalog) |
