---
mode: "wide"
title: "Bulk transactions"
sidebarTitle: "Bulk transactions"
api: "POST /transactions/bulk"
description: "Submit multiple transactions in one request with the Python SDK."
---

import NeedHelp from "/snippets/need-help.mdx";
import { RelatedTopics } from "/snippets/related-topics.jsx";

Use `blnk.transactions.create_bulk` to create many [transactions](/transactions/bulk-transactions) in one request.

<Steps>
<Step title="Call the method">

<Tabs>
<Tab title="Create batch">

```python blnk.transactions.create_bulk wrap
response = blnk.transactions.create_bulk({
  "atomic": True,
  "transactions": [
    {
      "amount": 1000,
      "precision": 100,
      "currency": "USD",
      "source": "@source_1",
      "destination": "@dest_1",
      "reference": "bulk_001",
      "description": "Payment 1",
    },
    {
      "amount": 2000,
      "precision": 100,
      "currency": "USD",
      "source": "@source_2",
      "destination": "@dest_2",
      "reference": "bulk_002",
      "description": "Payment 2",
    },
  ],
})
```

| Field | Type | Description |
| :-- | :-- | :-- |
| `transactions` | `list[dict]` | Transaction objects to create; references must be unique within the batch. |
| `atomic` | `bool` | When `True`, every transaction succeeds or fails together. |
| `inflight` | `bool` | When `True`, transactions are created in `INFLIGHT` status for a later commit or void. |
| `run_async` | `bool` | When `True`, Core processes the batch in the background and returns `status: "queued"`. |
| `skip_queue` | `bool` | When `True`, transactions bypass the queue and process synchronously. |

</Tab>

<Tab title="Dry run">

Preview the batch without writing anything. See [Dry-run transactions](/transactions/dry-run).

```python blnk.transactions.create_bulk wrap {2}
preview = blnk.transactions.create_bulk({
  "dry_run": True,
  "skip_queue": False,
  "transactions": [
    {
      "precise_amount": 45000,
      "precision": 100,
      "currency": "USD",
      "source": "bln_source-bal-001",
      "destination": "bln_dest-bal-001",
      "reference": "ref_payout_450",
      "description": "First payout",
    },
  ],
})
```

</Tab>
</Tabs>

</Step>

<Step title="Save the batch ID">

On a posted batch, use `response.data["batch_id"]` as `parent_transaction` to find child transactions, or with update_status to commit or void an inflight batch.

A dry run has no `batch_id`. Check `would_apply` and `results`.

</Step>

<Step title="Response">

<Tabs>
<Tab title="Create batch">

<CodeGroup>
```json Sync wrap
{
  "batch_id": "bulk_c62f200b-905f-4983-a349-cadd279234aa",
  "status": "applied",
  "transaction_count": 4
}
```

```json Async (run_async: true) wrap
{
  "batch_id": "bulk_c62f200b-905f-4983-a349-cadd279234aa",
  "status": "queued",
  "message": "Bulk transaction processing started"
}
```
</CodeGroup>

</Tab>

<Tab title="Dry run">

```json 200 OK wrap expandable {2-3}
{
  "dry_run": true,
  "would_apply": true,
  "cumulative": false,
  "atomic": false,
  "results": [
    {
      "dry_run": true,
      "would_apply": true,
      "status": "APPLIED",
      "reference": "ref_payout_450",
      "currency": "USD",
      "amount": 450,
      "precise_amount": "45000",
      "precision": 100
    }
  ]
}
```

</Tab>
</Tabs>

| Field | Type | Description |
| :-- | :-- | :-- |
| `batch_id` | `str` | Identifier for the batch. Use as parent_transaction to find child transactions, or with update_status to commit or void an inflight batch. |
| `status` | `str` | `applied`, `inflight`, or `queued` depending on the batch. |
| `transaction_count` | `number` | Number of transactions when processed synchronously. |
| `message` | `str` | Status message when `run_async` is `True`. |

</Step>
</Steps>

---

<NeedHelp />

<RelatedTopics
  items={[
    { title: "Bulk transactions", href: "/transactions/bulk-transactions" },
    { title: "Handling hot balances", href: "/guides/hot-balances" },
    { title: "Create inflight", href: "/transactions/inflight/creating-inflight" },
    { title: "Bulk transactions API", href: "/reference/bulk-transactions" },
  ]}
/>
