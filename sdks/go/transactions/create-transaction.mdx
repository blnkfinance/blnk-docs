...
```go client.Transaction.Create wrap {14-16}
...
```go Multiple destinations wrap {8-23}
transaction, resp, err := client.Transaction.Create(blnkgo.CreateTransactionRequest{
	ParentTransaction: blnkgo.ParentTransaction{
		PreciseAmount: big.NewInt(3000000),
		Precision:     100,
...
```go client.Transaction.Create wrap {1,17}
...
```go client.Transaction.Create wrap {1,12}
