...
```go client.Transaction.Create wrap {15-17}
...
```go Multiple destinations wrap {10-24}
transaction, resp, err := client.Transaction.Create(
 blnkgo.CreateTransactionRequest{
	ParentTransaction: blnkgo.ParentTransaction{
	 PreciseAmount: big.NewInt(3000000),
...
| Field | Type | Description |
| :-- | :-- | :-- |
| `Sources` | `[]blnkgo.Source` | Source legs when debiting more than one balance. |
| `Destinations` | `[]blnkgo.Source` | Destination legs when crediting more than one balance. |
...
| `Distribution` | `blnkgo.Distribution` | Percentage, fixed amount, or `left` for a split leg. |
...
```go client.Transaction.Create wrap {18}
...
| `MetaData` | `MetaData` | Optional metadata stored on the scheduled transaction. |
...
```go client.Transaction.Create wrap {13}
...
Preview whether a transaction would apply and how it would change balances without writing anything. Use `CreateDryRun`. Do not set `DryRun: true` on `Create`. See [Dry-run transactions](/transactions/dry-run).

```go client.Transaction.CreateDryRun wrap
preview, resp, err := client.Transaction.CreateDryRun(
	blnkgo.CreateTransactionRequest{
		ParentTransaction: blnkgo.ParentTransaction{
			PreciseAmount: big.NewInt(12000),
			Precision: 100,
			Currency: "USD",
...
| Field | Type | Description |
| :-- | :-- | :-- |
| Method | `CreateDryRun` | Returns `*TransactionPreview`. Nothing is written and the reference is still free. |
...
| `Status` | `blnkgo.`<br />`PryTransactionStatus` | Transaction [status](/transactions/transaction-lifecycle#statuses) (for example `QUEUED`, `APPLIED`, `INFLIGHT`). |
...
| `Sources` | `[]blnkgo.Source` | Source legs for split transactions. |
| `Destinations` | `[]blnkgo.Source` | Destination legs for split transactions. |
| `Reference` | `string` | Unique reference for the transaction. |
| `Description` | `string` | Narration of the transaction. |
| `EffectiveDate` | `*time.Time` | Actual date the transaction occurred, for backdated transactions. |
