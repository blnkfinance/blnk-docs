...
On a posted update, check `transaction.Status` for `APPLIED` or `VOID`. The child gets a new `TransactionID`. Use `ParentTransactionID` to link it to the original inflight transaction.
...
| `Status` | `blnkgo.`<br />`PryTransactionStatus` | `APPLIED` or `VOID`. |
