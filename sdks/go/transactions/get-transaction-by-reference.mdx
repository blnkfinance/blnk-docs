...
description: "Retrieve a transaction by its reference with the Go SDK."
---

import NeedHelp from "/snippets/need-help.mdx";

Use `client.Transaction.GetByReference` to retrieve a transaction by reference.

<Steps>
<Step title="Call the method">

```go client.Transaction.GetByReference
transaction, resp, err := client.Transaction.GetByReference(
	"ref_04551509-d7d3-4eab",
)
```
...
Use `transaction.TransactionID` when you need to refund, update inflight status, or inspect lineage.
