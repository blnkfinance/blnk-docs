...
<Info>
`blnk-go@v1.5.0` targets Blnk Core **0.15.4**. See the [Changelog](/changelog/blnk-go) before upgrading.
</Info>
...
Create a helper that reads your base URL and API key from the environment, then passes functional options for timeout and retries:
...
Pass a request struct to the service method you need. For example, create a ledger with metadata:
...
Transaction `PreciseAmount` fields use `*big.Int` in the Go SDK. Pass minor units with `big.NewInt(...)`, for example `big.NewInt(75000)` with `Precision: 100` for 750.00. Add `import "math/big"` at the top of your file when you use `big.NewInt`.
