...
When the SDK cannot reach Core or the request times out, you get a network or timeout error from the HTTP client. Configure timeout and logging in [Using the SDK](/sdks/go/start/using-the-sdk#timeouts-and-retries) to investigate these failures.
...
if err != nil {
	...
	if errors.As(err, &apiErr) {
		// apiErr.Status == 404
		// string(apiErr.Body) may include error_detail.code on Core 0.15.0+
	}
}
```

Parse `error_detail.code` from `apiErr.Body` when you need code-level routing. Treat message strings as display text; Core may change wording between releases. See [API error codes](/advanced/error-codes) for every code and its meaning.

| Status | When it happens | What to do |
| :-- | :-- | :-- |
| `404` | Resource ID or path does not exist | Parse `error_detail.code` from the body when available (e.g. `TXN_NOT_FOUND`, `IDT_NOT_FOUND`) |
...
<Warning>
Do not match exact error message strings in application logic. Message text can change. Prefer `error_detail.code` from the response body for routing. See the [0.15.0 migration guide](/changelog/v15-migration) if you parse error text today.
</Warning>
...
When the SDK cannot reach Core or the request times out, you get a network or timeout error from `http.Client`. Configure timeout and logging in [Using the SDK](/sdks/go/start/using-the-sdk#timeouts-and-retries).
