...
| :--- | :--- | :--- |
| `create` | [Create transaction](/reference/create-transaction) | Post a transfer between balances |
| `get` | [Get transaction](/reference/get-transaction) | Fetch by `transaction_id` |
| `getByReference` | [Get by reference](/reference/get-transaction-by-reference) | Fetch by your `reference` |
| `getLineage` | [Get transaction lineage](/reference/get-transaction-lineage) | Fund allocation and shadow txs |
| `createBulk` | [Bulk transactions](/reference/bulk-transactions) | Submit a batch of transactions |
| `updateStatus` | [Update inflight status](/reference/update-inflight-status) | Commit or void inflight |
| `bulkCommitInflight` | [Bulk commit inflight](/reference/bulk-commit-inflight) | Commit up to 100 inflight txs |
| `bulkVoidInflight` | [Bulk void inflight](/reference/bulk-void-inflight) | Void up to 100 inflight txs |
| `recoverQueue` | [Queue recovery](/reference/queue-recovery) | Recover stuck queued transactions |
...
`blnk.Transactions.recoverQueue` manually triggers recovery of stuck queued transactions (`POST /transactions/recover`). Optionally pass a `threshold` duration (for example `5m`, `1h`).
...
| Parameter | Type | Required | Description |
| :--- | :--- | :--- | :--- |
| `options.threshold` | string | No | Age threshold for stuck transactions (for example `5m`, `1h`). |

See [Queue recovery](/reference/queue-recovery) for request fields, response shape, and validation.
