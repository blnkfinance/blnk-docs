...
<Info>
`@blnkfinance/blnk-typescript@1.5.0` targets Blnk Core **0.15.4**. See the [Changelog](/changelog/blnk-ts) before upgrading.
</Info>
...
Timeouts and retries ship in v1.2.0 or later. When connections fail, Core restarts, or requests hang, the SDK handles it without blocking your app. To configure, pass the following parameters in the second `options` object:
...
<RelatedTopics
  items={[
    { title: "Quick start", href: "/sdks/typescript/start/quick-start" },
    { title: "Error handling", href: "/sdks/typescript/start/error-handling" },
