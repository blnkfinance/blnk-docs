...
You can add metadata when [creating a resource](/metadata/create-metadata) or [update metadata](/metadata/update-metadata) on ledgers, transactions, identities, and balances after they have been created. When you update the metadata of a parent resource, the changes propagate to its children. For example, if you update the metadata of an inflight transaction, it is also updated on its applied or void child transactions.
