...
sidebarTitle: "Migration to 0.10.5"
icon: "arrow-right"
...
**Core 0.15.0** applies an equivalent index as `idx_txn_meta_queued_parent` (partial, where the key is present) plus related queue indexes. 

After you upgrade, you do not need to keep a separate hand-created index unless your operator process requires it.
...
3. Confirm child outcomes by filtering on `meta_data.QUEUED_PARENT_TRANSACTION`, or wait for webhook events.
