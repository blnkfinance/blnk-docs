...
<Note>
Upgrading from `0.14.0` to `0.15.5` means reading 0.14.3, 0.15.0, 0.15.1, then 0.15.4 before you deploy. 0.15.5 has no migration guide.
</Note>
...
| [0.15.4](/changelog/v15-4-migration) | Missing balances on transaction routes return `BAL_NOT_FOUND`; already-refunded returns `TXN_ALREADY_REFUNDED`; both `sources[]` and `destinations[]` is rejected with `400`; `server.ssl` starts HTTPS again. |
...
**Example:** on `0.12.1` going to `0.15.5`, read 0.13.2, then 0.14.0, then 0.15.0, then 0.15.1, then 0.15.4. Skip guides at or below your current version. 0.15.5 has no migration guide.
</Step>
...
For **0.15.3 and later**, pull `ghcr.io/blnkfinance/blnk:<version>`. For **0.15.2 and earlier**, keep `jerryenebeli/blnk:<version>`.
</Step>
