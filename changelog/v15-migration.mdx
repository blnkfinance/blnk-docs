...
| Reconciliation start response changed | Integrations that read reconciliation status from the start response | Start endpoints now return only `reconciliation_id`. |
...
## Reconciliation start response changed
...
Both endpoints now return only the `reconciliation_id`.

If your code reads `status`, `matched_transactions`, or similar fields from the start response, move that logic to [Get reconciliation](/reference/get-reconciliations). Here's how it works now with 0.15.0:

1. Start the reconciliation.
2. Save the returned reconciliation_id.
3. Poll `GET /reconciliation/{reconciliation_id}`.
4. Continue polling until status is completed or failed.
5. Read match counts and results from the GET response.

Save `reconciliation_id` from the start call, then poll `GET /reconciliation/{reconciliation_id}` until `status` is `completed` or `failed`.

1. Move that logic to [Get reconciliation](/reference/get-reconciliations).
2. Save `reconciliation_id` from the start call.
3. Poll `GET /reconciliation/{reconciliation_id}` until `status` is `completed` or `failed`.

For batch runs, early polls may return `status: "in_progress"`.

For instant runs, the job may already be complete by the time you poll. You still need the GET call to read match counts and verify the result.

See [Reconciliations](/reconciliations/overview) documentation.
...
- Update reconciliation flows to fetch status using `reconciliation_id`.
