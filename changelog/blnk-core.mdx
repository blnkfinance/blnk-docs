...
<Update label="0.15.0" description="Jul 6, 2026">
  ...
  2. **Reconciliation webhooks:** Blnk now sends `reconciliation.completed` and `reconciliation.failed` webhooks when a run completes or fails. [Start reconciliation](/reference/start-reconciliation) and [Instant reconciliation](/reference/instant-reconciliation) now return only `reconciliation_id` in the start response. [0.15.0 migration guide →](/changelog/v15-migration#reconciliation-response-changed) · [Webhooks →](/advanced/notifications)
...
<Update label="0.14.3" description="May 26, 2026">
  ...
  1. **Hook management:** Creating, updating, viewing, listing, and deleting [transaction hooks](/hooks/create-hooks) now requires the master key. Regular API keys return `403 Forbidden`.
</Update>
