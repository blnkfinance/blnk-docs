...
<Update label="0.15.4" description="Sep 4, 2026">
  ...
  `server.ssl` is honoured again. Through 0.12.x the flag worked. From 0.13.0 through 0.15.3 it was ignored and Core always served HTTP.

  `ssl: true` now starts HTTPS on `server.port`. If a certificate cannot be issued, the server fails to start.
