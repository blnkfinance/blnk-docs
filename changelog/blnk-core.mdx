...
<Update label="Jul 10, 2025" description="v0.10.6">
  ## Redis TLS verification control

  The Redis configuration now supports a `skip_tls_verify` option. This allows you to connect to Redis instances with self-signed or untrusted certificates by skipping TLS certificate verification. This is useful for certain managed Redis services or development environments.
