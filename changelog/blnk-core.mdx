...
<Update label="0.15.5" description="Sep 24, 2026">
  ## Metadata update webhooks

  You can now receive a webhook when you update metadata on a ledger, balance, identity, or transaction. After a successful public update, Blnk sends `ledger.metadata.updated`, `balance.metadata.updated`, `identity.metadata.updated`, or `transaction.metadata.updated`.

  [Supported events →](/webhooks/events#metadata)

  ## Fixes & improvements

  1. **Queued effective date:** Applied transactions now keep `effective_date` when the request went through the queue. GET by id and by reference also return the field. [Backdated transactions →](/transactions/backdated-transactions)
  2. **Redis idle connections:** Pooled Redis connections now recycle before managed Redis or Valkey close them. Set `BLNK_REDIS_CONN_MAX_IDLE_TIME` if you need a different idle window (default 2 minutes). [Redis configuration →](/advanced/configuration/data-stores#redis-configuration)
  3. General bug fixes and improvements.
</Update>

<Update label="0.15.4" description="Sep 4, 2026">
  ## HTTPS (`server.ssl`)

  `ssl: true` now starts HTTPS on `server.port` again. If a certificate cannot be issued, the server fails to start.

  <Warning>
    If you left `ssl: true` while talking HTTP, set it to `false` before you upgrade, or follow [Enable HTTPS](/advanced/enable-https).
  </Warning>

  [0.15.4 migration guide →](/changelog/v15-4-migration#serverssl-starts-https)

  ## Fixes & improvements

  1. **Split validation:** A transaction that includes both `sources[]` and `destinations[]` is rejected with `400 TXN_VALIDATION_ERROR` instead of a misleading `404`. [0.15.4 migration guide →](/changelog/v15-4-migration)
  2. **Missing balance on a transaction route:** A source or destination that does not exist now returns `404 BAL_NOT_FOUND` instead of `TXN_NOT_FOUND`.
  3. **Already-refunded:** A second refund of the same transaction returns `409 TXN_ALREADY_REFUNDED` instead of `GEN_CONFLICT`, and no longer reverses the original refund.
  4. **Mixed refund batches:** When some legs of a parent refund fail for reasons other than already-refunded, the batch returns `409 GEN_CONFLICT`.
  5. **Split attribution:** Multi-source and multi-destination legs keep a stable `transaction_id` and reference suffix that match the order you sent.
  6. **Database connect:** Blnk retries the initial Postgres connection instead of exiting on the first failure.
  7. **Balance snapshots:** The snapshot bound and the caller's cutoff now use the same clock as the columns they bound.
  8. **Queue recovery:** Zero-amount transactions stuck in `QUEUED` can be recovered.
  9. **Error messages:** Not-found errors name the resource that was missing. Request decode errors no longer leak Go type names.
  10. General bug fixes and improvements.
</Update>

<Update label="0.15.3" description="Aug 25, 2026">
  ## Docker image update

  The official Core image is now published at `ghcr.io/blnkfinance/blnk`. 
  
  We recommend using this registry for new deployments starting from **0.15.3**. For **0.15.2 and earlier**, keep using `jerryenebeli/blnk`.

  ## Internal balances

  You can now create General Ledger balances yourself with an explicit `indicator`. To create, send `ledger_id: "general_ledger_id"` and your **@** `indicator` on `POST /balances`. 

  ## Fixes & improvements

  1. **Dry-run transactions:** You can now preview a transaction's effect on balances without writing anything to the ledger. Set `dry_run: true` on create, bulk create, refund, and inflight update.
  2. **Refund description and metadata:** You can now pass `description` and `meta_data` when you [create a refund](/transactions/refunds). 
  3. **Hooks:** Listing hooks without a type now returns all hooks.
  4. **Validation:** Negative amount or precision, and source equal to destination, are now rejected before processing.
  5. **Scoped keys can update metadata:** You can now use a scoped key with `metadata:write` to update metadata.
  6. **Inflight metadata:** Committed and voided transactions no longer keep `inflight: true` in their response metadata.
  7. **Balance search metadata:** After a transaction, Typesense no longer wipes balance `meta_data`. The search index now keeps the same metadata stored in Postgres.
  8. General bug fixes and improvements.
</Update>

<Update label="0.15.2" description="Jul 31, 2026">
  ## Fixes & improvements

  1. **Queue backpressure:** Blnk now rejects new queue enqueues when Redis memory usage or pending task count exceeds configured limits. Affected requests return `503 Service Unavailable` with `error_detail.code` set to `QUEUE_BACKPRESSURE`. Backpressure is enabled by default. [Learn more →](/advanced/configuration/queue)
  2. **Typesense memory backpressure:** Reindex and search indexing now retry gracefully when Typesense hits memory limits instead of failing immediately. [Reindex Typesense →](/search/typesense/reindex)
  3. **Typesense reindex:** `effective_date` is now preserved when rebuilding the Typesense index from Postgres.
  4. General bug fixes and improvements.
</Update>

<Update label="0.15.1" description="Jul 17, 2026">
  ## Fixes & improvements

  1. **Transaction lock duration:** Fixed an overflow bug when applying `BLNK_TRANSACTION_LOCK_DURATION`. [0.15.1 migration guide →](/changelog/v15-1-migration)
  2. **Transaction filter:** `POST /transactions/filter` now returns `parent_transaction` instead of an empty string.
  3. **Queued inflight commits:** Committing against a multi-leg parent transaction id now works when the request goes through the queue.
  4. **Search/reindex:** Transaction search and reindex now include `parent_transaction`, `precise_amount`, and `precision`.
  5. **Reconciliation uploads:** Improved reconciliation file upload handling and cleaned up file-handle closing.
  6. General bug fixes and improvements.
</Update>

<Update label="0.15.0" description="Jun 23, 2026">
  ## Structured API errors

  Error responses include a structured `error_detail` object with a stable code and clear messages. The response also includes the `error` field.
  ...
  <Warning>
    If your app checks status codes or error message text, review the [0.15.0 migration guide](/changelog/v15-migration) before upgrading.
  </Warning>

  [API error codes →](/advanced/error-codes)

  ## Removed `rate`, `currency_multiplier`, and `modification_ref`

  Transaction and balance responses no longer include `rate`, `currency_multiplier`, or `modification_ref`. Request `rate` no longer applies an exchange rate; destination credit equals source debit.

  <Warning>
    If your app reads these fields or uses request `rate` for FX, review the [0.15.0 migration guide](/changelog/v15-migration#rate-currency_multiplier-and-modification_ref-removed) before upgrading.
  </Warning>

  ## Queued inflight commit and void

  Commit and void for inflight transactions now go through the queue by default. Bulk commit and void also follow the same default. 
  
  Enable `skip_queue` in the request body when you need the previous synchronous behavior.

  [Commit & void inflight →](/transactions/inflight/updating-inflight)

  ## Transaction hash chain

  Blnk now supports an optional global hash chain to detect tampering in your transaction history and confirm ledger integrity on demand. 
  
  It is disabled by default; enable it in config and run `blnk verify-chain` from your Core deployment.

  [Transaction hashing →](/transactions/hash)

  ## Fixes & improvements

  1. Added DELETE endpoints for balance monitors and [identities](/reference/delete-identity).
  2. **Reconciliation webhooks:** Blnk now sends `reconciliation.completed` and `reconciliation.failed` webhooks when a run completes or fails. [Start reconciliation](/reference/start-reconciliation) and [Instant reconciliation](/reference/instant-reconciliation) now return only `reconciliation_id` in the start response. [0.15.0 migration guide →](/changelog/v15-migration#reconciliation-response-changed) · [Supported events →](/webhooks/events#reconciliations)
  3. Added a `system.error` webhook for internal failures during async processing.
  4. Increased default worker and database pool limits. 
  5. Added configurable [request and upload size caps](/advanced/configuration/server-security#request-and-payload-limits). 
  6. [Bulk create](/reference/bulk-transactions) and [instant reconciliation](/reference/instant-reconciliation) now enforce max **10,000** items per request.
  7. General bug fixes and performance improvements.
</Update>

<Update label="0.14.5" description="Jun 8, 2026">
  ## Fixes & improvements

  1. **Bulk transaction validation:** The [Create bulk transactions](/reference/bulk-transactions) endpoint now validates each item in the `transactions` array before processing. Invalid fields return `400 Bad Request` with indexed error messages.
  2. **Logging levels:** Transaction processing, reconciliation progress, and search indexing messages that reflect normal workflow progress are now logged at **info** level instead of **error**, so operational logs are easier to filter and alert on.
</Update>

<Update label="0.14.4" description="Jun 6, 2026">
  ## Remote monitoring exporter

  Blnk now supports optional remote export of logs, traces, and metrics through a Cloud DSN. Set **`BLNK_CLOUD_DSN`** to forward observability data to your Blnk Cloud monitoring project. Exported logs have sensitive values redacted.

  [Monitoring in Blnk →](/advanced/monitoring)

  ## Fixes & improvements

  1. **Scheduled inflight commits:** Fixed a bug where [scheduled inflight commits](/transactions/inflight/updating-inflight#schedule-inflight-commits) did not commit at the time you set. Transactions with `inflight_commit_date` now commit automatically as expected.
  2. **Typesense search:** Fixed a bug that prevented transactions from reliably indexing into [Typesense search](/search/typesense/introduction), so recorded transactions could not be found when searching.
</Update>

<Update label="0.14.3" description="May 26, 2026">
  ## Security hardening

  When you manage API keys with a non-master key, Blnk now limits you to your own `owner_id`: you can create, list, and revoke keys for that owner only, and you cannot grant scopes you don't already have. 
  
  The master key still has full access to create, list, and revoke keys for any owner.

  Hook management (create, update, view, list, and delete) now requires the master key. Regular API keys return `403 Forbidden`.

  <Warning>
    These auth rules can break integrations that manage hooks or cross-owner API keys with a scoped key. Review the [0.14.3 migration guide](/changelog/v14-3-migration) before upgrading.
  </Warning>

  [Owner context →](/api-keys/owner-context) · [Transaction hooks →](/webhooks/transaction-hooks)
</Update>
...
<Update label="0.14.1" description="May 6, 2026">
  ...
  [Schedule inflight commits →](/transactions/inflight/updating-inflight#schedule-inflight-commits)
</Update>
...
<Update label="0.14.0" description="Apr 9, 2026">
  ...
  [Transaction configuration →](/advanced/configuration/transactions) 
  
  [Queue configuration →](/advanced/configuration/queue)

  ## Fixes & improvements

  1. **Update Metadata response:** The [Update Metadata](/reference/update-metadata) endpoint now returns **`meta_data`** in the response instead of `metadata`. [View migration guide →](/changelog/v14-migration)
</Update>
...
<Update label="0.13.2" description="Feb 12, 2026">
  ...
  1. **Unique reference constraint:** Blnk now enforces a unique database index on `transactions.reference` (`idx_transactions_reference_unique`), strengthening idempotency guarantees at the storage layer.

  <Warning>
    This release includes a breaking database change: transaction `reference` values must be unique. Upgrade fails if your database already contains duplicate references. 
    
    Review the [0.13.2 migration guide](/changelog/v13-2-migration) before upgrading.
  </Warning>
</Update>
...
<Update label="0.11.5" description="Nov 28, 2025">
  ...
  2. You can now tune webhook processing performance with the new [`webhook_concurrency`](/advanced/configuration/queue#queue-settings) configuration.
</Update>
...
<Update label="0.10.5" description="Jul 4, 2025">
  ...
  <Warning>
  This is a breaking change if your app expects bulk create to finish synchronously. See the [0.10.5 migration guide](/changelog/v10-5-migration).
  </Warning>
...
<Update label="0.10.1" description="Mar 22, 2025">
  ...
  [Set your API keys →](/api-keys/overview)
...
<Update label="0.8.1" description="Jan 18, 2025">
  ...
  <Card title="How transactions work in Blnk" href="/transactions/introduction" icon="tag">
    Recording, sources, destinations, and statuses.
  </Card>
  ...
  <Card title="Configuration file" href="/advanced/configuration/overview" icon="settings">
    blnk.json options and environment variables.
  </Card>
