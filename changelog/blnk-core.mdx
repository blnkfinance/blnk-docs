...
<Update label="Mar 16, 2025" description="v0.10.0">
  ...
  Introducing the Overdraft Limits feature, designed to give you greater control when you apply overdrafts to transactions. By enabling the `allow_overdraft` parameter, you can now allow account balances to go negative, with the flexibility to set a maximum overdraft threshold using the `overdraft_limit` parameter. 
  ...
  [Set Overdraft Limits →](/transactions/overdrafts#overdraft-limit)

  ## Access Control

  Using your master key, you can now create API keys with specific permissions to enforce granular access control in your ledger. 
