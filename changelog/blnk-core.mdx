...
<Update label="0.15.0" description="Jul 6, 2026">
  ...
  2. Removed `rate`, `currency_multiplier`, and `modification_ref` from API responses.
  ...
  7. Changed reconciliation start endpoints to return only `reconciliation_id`; poll for full status.
...
<Update label="0.14.4" description="Jun 6, 2026">
  ...
  Blnk now supports optional remote export of logs, traces, and metrics through a monitoring DSN. Set **`BLNK_MONITORING_DSN`** to forward observability data to your monitoring backend. Exported log payloads redact sensitive fields, bearer tokens, DSNs, and query values.

  [Observability configuration →](/advanced/configuration/observability)
