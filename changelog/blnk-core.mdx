...
<Update label="0.14.3" description="May 26, 2026">
  ...
  This release tightens authorization around API key and hook management.

  ## Fixes & improvements

  1. **API key authorization:** API keys can now manage only their own owner context. Non-master API keys cannot manage keys for another owner or grant scopes broader than the scopes they already have. The master key can still create, list, and revoke keys across owners.
  2. **Hook management:** Creating, updating, viewing, listing, and deleting [transaction hooks](/hooks/create-hooks) now requires the master key. Requests made with regular API keys return a forbidden response.
</Update>
