...
<Update label="1.5.0" description="Sep 24, 2026">
  ...
  Named codes now cover the rest of Core's catalogue. Branch on `ErrorDetail.Code`. See [API error codes](/advanced/error-codes).

  ### Search

  Search more than one collection in a single request with `Search.MultiSearch`. See [Multi-search](/sdks/go/search/multi-search).
