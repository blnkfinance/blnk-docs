...
<Update label="1.5.0" description="Sep 24, 2026">
  <Info>
  `blnk-go@v1.5.0` targets Blnk Core **0.15.4**.
  </Info>

  ### Error handling

  Exported `ErrorCode*` constants now cover the Core 0.15.4 catalogue (78 codes). Branch on `ErrorDetail.Code`. See [API error codes](/advanced/error-codes) and the [0.15.4 migration guide](/changelog/v15-4-migration).
  ...
  `ACC_GENERATION_FAILED` is not exported. Account handlers only return `ACC_NOT_FOUND` and `ACC_DUPLICATE`.

  List and `MultiSearch` methods are unchanged from earlier releases.

  ### Before you upgrade

  - You need Core **0.15.4** for these error codes.
</Update>

<Update label="1.4.0" description="Sep 2, 2026">
  <Info>
  `blnk-go@v1.4.0` targets Blnk Core **0.15.3**.
  </Info>

  ### Transactions

  Preview a create, bulk, refund, or inflight update without writing anything. Use the dry-run methods such as `CreateDryRun` and `RefundDryRun`. Core returns whether it would apply and how balances would change. The reference is still free to use on a real post. See [Dry-run transactions](/transactions/dry-run).
  ...
  Named codes for invalid amounts, validation failures, and General Ledger conflicts are exported as constants. Branch on `ErrorDetail.Code`.

  ### Before you upgrade

  - You need Core **0.15.3** for these features.
  - Call the dry-run methods for previews. Do not set `DryRun: true` on the posted methods.
</Update>
