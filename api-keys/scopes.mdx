...
| Metadata updates | `metadata:write` | Update metadata on ledgers, transactions, identities, and balances |
...
<Note>
`POST /{id}/metadata` maps to the `metadata` resource. A scoped key needs `metadata:write` to [update metadata](/metadata/update-metadata).
</Note>
