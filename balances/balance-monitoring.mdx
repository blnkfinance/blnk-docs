...
<Step title="Define your condition">
  Choose the balance to watch, the sub-balance field, the comparison operator, and the threshold value. Use the same `precision` as the balance you are monitoring.
...
| `precision` | Precision applied to `value`. Must match the monitored balance. |
