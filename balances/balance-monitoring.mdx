...
ApiResponse<JsonNode> response = blnk.balanceMonitor().create(
    MonitorData.create()
        .balanceId("bln_0be360ca-86fe-457d-be43-daa3f966d8f0")
        .condition(MonitorCondition.create()
            .field("debit_balance")
            .operator(">")
            .value(100000)
            .precision(100))
...
| `precision` | Precision applied to `value`. Must match how you record transactions for that currency. Cannot be negative. `0` means not supplied and is treated as `1`. |
...
```java Java wrap
ApiResponse<JsonNode> response = blnk.balanceMonitor().get("mon_e0e77b0c-4985-472a-9bf5-76a48b0259b0");
```
</CodeGroup>
...
```typescript TypeScript wrap
const response = await blnk.BalanceMonitor.listByBalanceId(
  '{balance_id}',
);
```

```go Go wrap
monitors, resp, err := client.BalanceMonitor.ListByBalanceID(
    "{balance_id}",
)
```

```python Python wrap
response = blnk.balance_monitor.list_by_balance_id(
  "{balance_id}",
)
```

```java Java wrap
ApiResponse<JsonNode> response = blnk.balanceMonitor().listByBalanceId(
    "{balance_id}");
```
...
```python Python wrap
response = blnk.balance_monitor.delete(
  "mon_e0e77b0c-4985-472a-9bf5-76a48b0259b0",
)
```

```java Java wrap
ApiResponse<JsonNode> response = blnk.balanceMonitor().delete(
    "mon_e0e77b0c-4985-472a-9bf5-76a48b0259b0");
```
