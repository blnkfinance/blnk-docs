...
<CodeGroup>
```bash cURL wrap
curl -X GET "http://YOUR_BLNK_INSTANCE_URL/balance-monitors/{monitor-id}" \
  -H "X-blnk-key: <api-key>"
```
...
<Tip>
    List all balance monitors with `GET http://YOUR_BLNK_INSTANCE_URL/balance-monitors`:
</Tip>
...
and provide the updated conditions in the request body.
