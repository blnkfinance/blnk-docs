...
2. **Always [apply rates](/transactions/rates) for transactions between currencies:** Specify a `rate` value for transactions involving different currencies to ensure the destination balance is calculated accurately. Without a set exchange rate, discrepancies can arise, leading to inaccurate ledger balances and potential financial mismatches.
