...
Call the [Create internal balance](/reference/create-internal-balance) endpoint with `ledger_id` set to `general_ledger_id` and the `indicator` you want to use.

Continuing our earlier example, create `@Revenue` like this:
...
<Info>
    On Blnk Core 0.15.2 and earlier, pass your preferred `@` indicator as the `source` or `destination` of a transaction. Blnk creates the General Ledger balance automatically.
</Info>
...
<RelatedTopics
  items={[
    ...
    { title: "Create internal balance", href: "/reference/create-internal-balance" },
