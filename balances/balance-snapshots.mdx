...
Call the [Balance Snapshots](/reference/balance-snapshots) endpoint. This action initiates a snapshot of all balances, capturing their current state at the time of the request for future reference.

```
POST YOUR_BLNK_INSTANCE_URL/balances-snapshots
```
``` json Response
{
...
<Card title="Historical balances" iconType="duotone" icon="rectangle-history" href="/balances/historical-balances">
  Learn how to use balance snapshots to retrieve balances at a point in time.
</Card>
