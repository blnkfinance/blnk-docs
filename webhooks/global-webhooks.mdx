...
<Step title="Set your webhook URL">
  ...
  <Note>
  When using `.env`, set `BLNK_WEBHOOK_HEADERS` to a JSON-encoded object. For `blnk.json`, set `notification.webhook.headers` to a JSON object. See [Webhooks configuration](/advanced/configuration/webhooks#blnk_webhook_headers) for details.
  </Note>
</Step>
