...
icon: "loader"
"og:title": "Building a Escrow Payment Workflow with Blnk • Blnk Developer Documentation"
...
In this tutorial, we’ll build a basic escrow payment app using Blnk’s inflight transaction feature. Here's what we'll do:
...
4. Handle edge cases like partial payments.
...
For our escrow payments workflows, here's our map:
...
* Transaction is completed, and both balances get updated.
...
2. Trigger the escrow monitoring service to confirm the state of the escrow payment.
3. Commit or void the inflight transaction based on the status of escrow payment.
...
<Note>
  When `inflight` is enabled, the transaction remains pending, and neither the source nor destination balances are affected until the specified conditions are met.

  With the [inflight feature](/transactions/inflight), the ledger can keep track of both successful and failed escrow payments on customers' balances.
</Note>
...
Escrow payments can also be fufilled in bits, i.e., you complete part of the amount per time. This is ideal for use cases like instalments, buy now pay later, etc.
...
<Tip>
  You can commit partial amounts, as long as they are less than the original transaction amount. Additionally, you can make multiple partial commits and void the remaining inflight amount if needed.
</Tip>
...
With Blnk's flexible architecture, you can easily adapt this escrow system to various business models - from marketplace transactions and freelancer payments to real estate deals and complex business contracts.
