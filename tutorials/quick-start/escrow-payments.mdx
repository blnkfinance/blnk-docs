...
func createEscrowTransaction(
    buyerWalletID, sellerWalletID string,
    amount float64,
    ...
    escrow, _, err := client.Transaction.Create(
...
ApiResponse<JsonNode> escrow = blnk.transactions().create(
...
System.out.println(
    "Escrow transaction created: " + escrow.data().get("transaction_id").asText()
);
```
</CodeGroup>
...
func commitInflightTransaction(transactionID string) (*blnkgo.Transaction, error) {
    client := getClient()
    committed, _, err := client.Transaction.Update(
        transactionID,
        blnkgo.UpdateStatus{
...
func voidInflightTransaction(transactionID string) (*blnkgo.Transaction, error) {
    client := getClient()
    voided, _, err := client.Transaction.Update(
        transactionID,
        blnkgo.UpdateStatus{
...
```go Go wrap
func commitPartialInflightTransaction(
...
ApiResponse<JsonNode> response = blnk.transactions().updateStatus(
    ...
    UpdateTransactionStatus.create()
        .status("commit")
        .preciseAmount((partial_amount * 100))
);
```
</CodeGroup>
