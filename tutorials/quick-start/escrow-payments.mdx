...
```javascript TypeScript wrap expandable {13}
...
```bash cURL wrap
curl --request PUT \
  --url http://localhost:5001/transactions/inflight/<inflight-transaction-id> \
  --header 'X-blnk-key: <api-key>' \
  --header 'Content-Type: application/json' \
  --data '{
    "status": "commit"
...
async function commitInflightTransaction(transactionId) {
  ...
  try {
    const response = await Transactions.updateStatus(transactionId, {
      status: 'commit',
    });
