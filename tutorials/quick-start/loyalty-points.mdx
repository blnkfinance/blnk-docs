...
2. **Redeeming points:**
    ...
    flowchart LR
        ...
        subgraph cash [Customer Purchase]
            customerWallet -- customer purchase<br/>(discounted) --> revenue
        end
        subgraph points [Redeem Points]
            customerPoints -- pay with points --> pointsRevenue
        end

        cash -- completed purchase --> points
...
To redeem points for a customer during a purchase:

<CodeGroup>
```javascript TypeScript
async function redeemPointsForDiscount(mainWalletId, pointsWalletId, purchaseAmount, pointsToRedeem, discountAmount) {
  const blnk = await getBlnkInstance();
  const { Transactions } = blnk;
