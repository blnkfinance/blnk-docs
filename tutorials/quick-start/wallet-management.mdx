...
func createCustomerLedger() (string, error) {
    client := getClient()
    ledger, _, err := client.Ledger.Create(
...
print("Customer Ledger created:", customer_ledger.data["ledger_id"])
```
...
System.out.println(
    "Customer Ledger created: " + customer_ledger.data().get("ledger_id").asText()
);
```
</CodeGroup>
...
func createCustomerIdentity(customerData CustomerData) (string, error) {
    client := getClient()
    identity, _, err := client.Identity.Create(
        blnkgo.Identity{
...
print("Customer Identity created:", identity.data["identity_id"])
```
...
System.out.println(
    "Customer Identity created: " + identity.data().get("identity_id").asText()
);
```
</CodeGroup>
...
print("Main Wallet created:", main_wallet.data["balance_id"])
```
...
System.out.println(
    "Main Wallet created: " + main_wallet.data().get("balance_id").asText()
);
```
</CodeGroup>
...
func depositToWallet(
    ...
    deposit, _, err := client.Transaction.Create(
...
print("Deposit transaction created:", deposit.data["transaction_id"])
```
...
System.out.println(
    "Deposit transaction created: " + deposit.data().get("transaction_id").asText()
);
```
</CodeGroup>
...
func withdrawFromWallet(
    ...
    withdrawal, _, err := client.Transaction.Create(
        blnkgo.CreateTransactionRequest{
            ParentTransaction: blnkgo.ParentTransaction{
                PreciseAmount: amount * 100,
                Precision: 100,
...
```python Python wrap expandable
withdrawal = blnk.transactions.create({
  "precise_amount": 10000,
  "precision": 100,
  "reference": "WDR-20250225-104500",
  "description": "Wallet withdrawal",
...
print("Withdrawal transaction created:", withdrawal.data["transaction_id"])
```

```java Java wrap expandable
ApiResponse<JsonNode> withdrawal = blnk.transactions().create(
    CreateTransactions.create()
        .preciseAmount(10000)
        .precision(100)
        .reference("WDR-20250225-104500")
...
System.out.println(
    "Withdrawal transaction created: " + withdrawal.data().get("transaction_id").asText()
);
```
</CodeGroup>
...
```python Python wrap expandable
card_wallet = blnk.ledger_balances.create({
  "ledger_id": ledger_id,
  "identity_id": identity_id,
  "currency": currency,
  "meta_data": {
...
print("Card Wallet created:", card_wallet.data["balance_id"])
```

```java Java wrap expandable
ApiResponse<JsonNode> card_wallet = blnk.ledgerBalances().create(
    CreateLedgerBalance.create()
        .ledgerId(ledger_id)
        .identityId(identity_id)
        .currency(currency)
        .metaData(Map.ofEntries(
            Map.entry("wallet_type", "card"),
            Map.entry("purpose", "card_payments"),
            Map.entry("status", "active"),
            Map.entry("card_details", Map.of(
                "masked_number", "xxxx-xxxx-xxxx-1234",
                "expiry", "12/25",
                "type", "virtual"
            ))
...
System.out.println(
    "Card Wallet created: " + card_wallet.data().get("balance_id").asText()
);
```
</CodeGroup>
...
```typescript TypeScript wrap expandable
async function transferToCardWallet(
  mainBalanceID,
  cardBalanceID,
  amount,
  uniqueReference,
...
```go Go wrap expandable
func transferToCardWallet(
    mainBalanceID, cardBalanceID string,
    amount float64,
    uniqueReference, description string,
) (string, error) {
    client := getClient()
    ...
    transfer, _, err := client.Transaction.Create(
        blnkgo.CreateTransactionRequest{
            ParentTransaction: blnkgo.ParentTransaction{
                PreciseAmount: amount * 100,
                Precision: 100,
...
```python Python wrap expandable
transfer = blnk.transactions.create({
  "precise_amount": 20000,
  "precision": 100,
  "reference": "TRF-20250225-110000",
  "description": "Transfer to card wallet",
  "currency": "USD",
  "source": main_balance_id,
  "destination": card_balance_id,
  "meta_data": {
    "transaction_type": "internal_transfer",
    "purpose": "fund_card",
  },
...
print("Transfer transaction created:", transfer.data["transaction_id"])
```

```java Java wrap expandable
ApiResponse<JsonNode> transfer = blnk.transactions().create(
    CreateTransactions.create()
        .preciseAmount(20000)
        .precision(100)
        .reference("TRF-20250225-110000")
...
System.out.println(
    "Transfer transaction created: " + transfer.data().get("transaction_id").asText()
);
```
</CodeGroup>
