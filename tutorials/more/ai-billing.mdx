...
"og:title": "Building an AI Billing System with Blnk • Blnk Developer Documentation"
...
Usage-based billing is the standard pricing model for AI products. Each usage event has two components that must be tracked together:
...
<Note>
  The `precision` parameter converts the amount to its smallest unit (e.g., 100 converts dollar amounts to cents). This helps avoid rounding errors common with floating-point arithmetic.
</Note>
</Step>
</Steps>
...
<Note>
  The `precision` for tokens is set to 1 since tokens aren't measured in decimals. For USD, use precision 100 to convert dollars to cents.
</Note>

<Note>
  Use meaningful reference identifiers that include customer ID, model, timestamp, and transaction type for easy tracing.
</Note>
...
To enable postpaid, set `allow_overdraft: true` on the USD leg of the bulk transaction:
