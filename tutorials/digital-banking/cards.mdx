...
If you skip the queue, Blnk processes the authorization immediately. A successful authorization returns its final status in the API response.

| Status | Meaning |
| :-- | :-- |
| `INFLIGHT` | Customer has sufficient funds. The transaction is authorized and the amount is reserved. |

If authorization fails because of insufficient funds, Blnk returns `400 TXN_INSUFFICIENT_FUNDS`. Learn more: [Managing insufficient funds](/transactions/introduction#managing-insufficient-funds).
</Tab>
</Tabs>
</Step>
