...
1. **The balance total:** This is what you already know: one number per currency.
...
<Accordion title="Example breakdown">
  <Steps>
    <Step title="Funds enter the system">
      ...
      Jack's total balance is **\$150**. Because lineage is enabled, Blnk also records the composition of that balance internally: **\$100** from Stripe and **\$50** from PayPal.
    </Step>
    <Step title="Funds move between balances">
      Jack sends $40 to Alice. Blnk automatically:
...
You still create the same credits and debits. Lineage just makes each spend explainable.
...
4. When money leaves the system, the tag attribution is preserved on the spend.
