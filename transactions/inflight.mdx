...
<Frame>
<img src="/images/step-01-02-using-inflight.png" alt="Step 1: How a transaction is processed in Inflight" className="rounded-lg" />
</Frame>
...
```json Response
{
  ...
  "failed": 0,
  "results": [
    { "transaction_id": "txn_f482a1b3-6c2d-4e89-a17b-3d5e8f2a1c94", "status": "APPLIED" },
    { "transaction_id": "txn_c5d9e2a1-7b4f-4a3c-9e8d-1f6a2b4c8d30", "status": "APPLIED" }
  ]
}
```
...
```json Response
{
  ...
  "failed": 0,
  "results": [
    { "transaction_id": "txn_f482a1b3-6c2d-4e89-a17b-3d5e8f2a1c94", "status": "VOID" },
    { "transaction_id": "txn_c5d9e2a1-7b4f-4a3c-9e8d-1f6a2b4c8d30", "status": "VOID" }
  ]
}
```
...
Bulk requests: max **100** transactions. Each item is processed independently and the response includes `succeeded`, `failed`, and a `results` array with each transaction's status.
...
```go Go wrap
commitAt, _ := time.Parse(time.RFC3339, "2024-12-21T01:36:46+01:00")
transaction, resp, err := client.Transaction.Create(blnkgo.CreateTransactionRequest{
  ParentTransaction: blnkgo.ParentTransaction{
    Amount:      100,
    Precision:   100,
...
```go Go wrap
expiresAt, _ := time.Parse(time.RFC3339, "2024-12-21T01:36:46+01:00")
transaction, resp, err := client.Transaction.Create(blnkgo.CreateTransactionRequest{
  ParentTransaction: blnkgo.ParentTransaction{
    Amount:      100,
    Precision:   100,
...
```json transaction.applied expandable
{
    ...
    "data": {
        ...
        "currency": "GBP",
        "description": "Inflight commit webhook",
        "status": "APPLIED",
...
<Steps titleSize="h3">
    <Step title="Initiate inflight">
        ...
        <Note>
        Use the same [Create transaction](/reference/create-transaction) request as a single transfer, and set `"inflight": true` in the body. For multiple sources or destinations, include the `sources` or `destinations` array instead of `source` / `destination`.
        </Note>
