...
<Steps titleSize="h3">
    <Step title="Record transaction with Inflight">
        ```json Record inflight {9,10}
        {
            "amount": 100,
            "precision": 100,
    ...
    <Step title="Updating Inflight">
        ...
        * Resets inflight balances without affecting actual balances.
        ...
        An inflight transaction only gets expired when an `inflight_expiry_date` is specified in the request body. 

        * Indicates transaction remained `INFLIGHT` past specified expiry date.
        * Creates new record with `EXPIRED` status.
