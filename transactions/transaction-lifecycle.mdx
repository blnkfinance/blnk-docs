...
Every recorded transaction can go through multiple states in Blnk, and each state change is stored as a separate record in the database. This gives you complete traceability, allowing you to see the lifecycle of a transaction from initiation to completion.
...
| `REJECTED` | A recorded transaction was not processed due to an error, such as insufficient funds | When processing fails after Blnk records the request |
...
* `REJECTED`: When a recorded transaction is not processed because of an error such as insufficient funds.
...
<Tip>If the source has insufficient funds, the result depends on whether you used the queue. Learn more: [Managing insufficient funds](/transactions/introduction#managing-insufficient-funds).</Tip>
