...
Because transactions in Blnk are immutable, each status change creates a new transaction record. This new record references the previous one as its "parent".
...
5. If the inflight transaction is voided instead, a new transaction is created in the `VOID` state.
    ...
    <Check>When you use **`inflight_commit_date`**, Blnk commits automatically at that time; the resulting **`APPLIED`** transaction still uses the inflight record as its **`parent_transaction`**, just like a manual commit via [Update Inflight](/reference/update-inflight).</Check>
...
results, resp, err := client.Search.SearchDocument(
  blnkgo.SearchParams{Q: "<inflight_transaction_id>", QueryBy: "parent_transaction"},
  blnkgo.Transactions,
)
```
</CodeGroup>
  ...
  "hits": [
    {
      "document": {
        ...
        "parent_transaction": "<inflight_transaction_id>",
        "status": "APPLIED"
      }
    }
  ]
  ...
  "hits": [
    {
      "document": {
        ...
        "meta_data": {
          "QUEUED_PARENT_TRANSACTION": "<queued_parent_transaction_id>"
        },
        "status": "APPLIED"
      }
    }
...
### Queued parent for bulk/split transactions
...
Unlike the [inflight transaction](#inflight-transaction) example above, you can’t query by `parent_transaction` to get the final outcome of a split transaction. 

This is because the final `APPLIED` transactions aren’t directly linked to the original split transaction. Instead, they’re linked to the two individual `QUEUED` transactions created from it - each representing a separate outcome for Person A and Person B. 
