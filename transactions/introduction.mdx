...
<Steps titleSize="h3">
  <Step title="Transaction lifecycle">
    Every transaction begins in a `QUEUED` state. Within milliseconds of being queued, the system processes it and transitions it to one of three possible states:
  ...
  <Step title="Record creation and tracking">
    When a transaction moves to its next state (`INFLIGHT`, `APPLIED`, or `REJECTED`), the system creates a new record that maintains a connection to the original queued transaction. This connection is maintained in two ways:
    ...
    2. **Reference Tracking:** To ensure each record remains uniquely identifiable while preserving its relationship to other states, the system:
  ...
  <Step title="Tracking transaction status">
    ...
    1. **Reference Search:** Query transactions using the original reference. The system will return the `INFLIGHT` or `APPLIED` record associated with that reference.
        ```json
        {
          "q": "payment_abc",
          "query_by": "reference"
...
When a transaction is posted where the `source` balance has insufficient funds, Bink automatically handles this scenario to maintain ledger integrity. The system:
