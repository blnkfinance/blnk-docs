...
- **Skip queue:** When `skip_queue: true`, Blnk processes the request immediately and returns `400 TXN_INSUFFICIENT_FUNDS`. The rejected attempt is not recorded as a transaction, and the `reference` is not used. You can retry the same `reference` after the source has enough funds.
