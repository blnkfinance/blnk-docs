...
<Tabs>
    <Tab title="Request">
        ...
        | --- | --- | --- | ---
        | `amount` | Transaction amount. Send either `amount` or `precise_amount`, not both — see [Precision](/transactions/precision#using-both-precision-options-together). | Yes | `float`
