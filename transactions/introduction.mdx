...
<Tip>
To preview the same request without writing a transaction, set `dry_run` to `true`. See [Dry-run transactions](/transactions/dry-run).
</Tip>
...
With `skip_queue: true`, a successful request immediately returns an `INFLIGHT` or `APPLIED` transaction. If the source has insufficient funds, see [Managing insufficient funds](/transactions/introduction#managing-insufficient-funds).
...
3. **Insufficient funds with `skip_queue: true`:** The rejected attempt is not recorded. See [Managing insufficient funds](/transactions/introduction#managing-insufficient-funds).
...
- **Default queue:** The API first returns a `QUEUED` transaction. If processing fails because of insufficient funds, the worker records a separate `REJECTED` transaction linked to the queued transaction through `parent_transaction`. Blnk also sends a webhook with the rejection reason in `meta_data`.
- **Skip queue:** When `skip_queue: true`, Blnk processes the request immediately and returns `400 TXN_INSUFFICIENT_FUNDS`. The rejected attempt is not recorded as a transaction, and the `reference` is not used. You can retry the same `reference` after the source has enough funds. This behavior was confirmed on Blnk Core 0.15.0.
...
<CardGroup cols={2}>
  <Card title="Dry-run transactions" icon="flask-conical" href="/transactions/dry-run">
