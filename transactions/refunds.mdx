...
Blnk records the reversal as a new transaction; the original record is not modified. You can refund each original transaction only once. A second attempt returns `409 TXN_ALREADY_REFUNDED` and does not move balances.
...
You can optionally set `description` and `meta_data` on the refund. [Learn more](#description-and-metadata).
...
```json 201 Created (queued) {13} wrap
{
  "amount": 50,
  "amount_string": "50",
  "precision": 100,
...
```json 201 Created (synchronous) {13} wrap
{
  "amount": 50,
  "amount_string": "50",
  "precision": 100,
...
By default, a refund copies the original transaction's `description` and `meta_data`. You can set these on the refund itself.

- `description` replaces the inherited narration. An empty string is ignored.
- `meta_data` is merged onto the inherited metadata. Keys you send replace matching keys; other keys stay on the refund.
...
If the original transaction has `{"type": "deposit", "channel": "card"}` and you send `{"type": "refund", "reason": "goodwill"}`, the refund stores:
...
Refunds that cannot be applied return `404 Not Found` or `409 Conflict`.

From 0.15.4, a second refund of the same transaction returns `TXN_ALREADY_REFUNDED` instead of `GEN_CONFLICT`, and does not reverse the original refund. When you refund a parent and some legs fail for mixed reasons, Blnk returns `GEN_CONFLICT`.

| Code | When it happens |
| :-- | :-- |
| `TXN_NOT_FOUND` | No refundable transaction exists for the ID you sent. |
| `TXN_ALREADY_REFUNDED` | The original transaction was already refunded. |
| `GEN_CONFLICT` | A parent refund had mixed outcomes (some legs already refunded, others failed for a different reason). |
...
| Code | What to do |
| :-- | :-- |
| `TXN_NOT_FOUND` | Confirm the ID is the original `APPLIED` or queued parent, not a `VOID` child. |
| `TXN_ALREADY_REFUNDED` | Treat the refund as complete. Do not retry. |
| `GEN_CONFLICT` | Inspect the failed legs. Retry only legs that are still refundable. |

See [API error codes](/advanced/error-codes) for the full catalogue. If you upgrade from 0.15.3 or earlier, review the [0.15.4 migration guide](/changelog/v15-4-migration).
