...
<Info>
From 0.15.5, queued apply keeps `effective_date` on the APPLIED transaction when `skip_queue` is false, including coalescing and stuck-queue recovery. `GET /transactions/:id` and `GET /transactions/reference/:reference` return the field when it is set.
</Info>
