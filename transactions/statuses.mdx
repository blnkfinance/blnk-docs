...
1. `APPLIED`: Your transaction status is updated to "applied" when the balances of the source and destination of the transaction get updated, with the transaction amount debited and credited respectively. A transaction with the `APPLIED` is deemed to completed its lifecycle.
