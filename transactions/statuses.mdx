...
There are two main primary transaction statuses for any transaction in Blnk.

<Note>
  All transactions in Blnk go through a [concurrency-controlled queue system](/resources/concurrency). When first created, transactions enter with a `QUEUED` status. This temporary state is later updated to one of two final statuses based on the transaction's processing outcome.
</Note>

1. `APPLIED`: Your transaction status is updated to "applied" when the balances of the source and destination of the transaction get updated, with the transaction amount debited and credited respectively. A transaction with the `APPLIED` has completed its lifecycle.

2. `REJECTED`: Your transaction status is updated to "rejected" if any of the conditions required for a successful transaction hasn't been met.

## 2. Inflight statuses

[Inflight](inflight) is a feature that allows you to set conditions to be met before a transaction is applied in your ledger. It is also known as "2-phase commit."
