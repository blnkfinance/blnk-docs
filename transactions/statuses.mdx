...
If you send a void command, Bink creates a new record with the `VOID` status, indicating you've chosen not to complete the transaction.

### Expired status: 

If no action is taken before the transaction's expiry time, Bink automatically creates a new record with the `EXPIRED` status. This indicates the transaction can no longer be committed or voided.
