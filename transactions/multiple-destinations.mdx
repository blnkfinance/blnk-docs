...
<Warning>
When sending to multiple destinations, you can only send from one source. A payload that includes both `sources[]` and `destinations[]` is rejected with `400 TXN_VALIDATION_ERROR`.
</Warning>
...
To preview the split without writing it, set `dry_run` to `true`. See [Dry-run transactions](/transactions/dry-run).
...
transaction, resp, err := client.Transaction.Create(blnkgo.CreateTransactionRequest{
    ParentTransaction: blnkgo.ParentTransaction{
        ...
        Destinations: []blnkgo.Source{
            {Identifier: "bln_f2073f6b-905a-4e3e-b5a2-8d1b3dc2fb7f", Distribution: "20%"},
            {Identifier: "bln_64c50fb5-32d5-4f78-9f4a-e8b01aaf025d", PreciseDistribution: "1000000"},
            {Identifier: "bln_7d98dfe9-5c3e-4c9b-b96a-65f6d9f7b89b", Distribution: "left"},
        },
    },
})
...
| :--- | :--- | :--- | :--- |
| `precise_amount` | Total amount to be sent in its smallest unit. Send either `precise_amount` or `amount`, not both - see [Precision](/transactions/precision#using-both-precision-options-together). | Yes | `integer` |
| `amount` | Total amount to be sent as a float. Blnk multiplies by `precision` to store `precise_amount`. | Yes | `float` |
...
curl -X POST "http://localhost:5001/search/transactions" \
  ...
  -H "Content-Type: application/json" \
  -d '{
    "q": "txn_0b59f6e6-6c4a-4efa-915c-526f77ef61ab",
  ...
  "hits": [
    {
      "document": {
        "transaction_id": "txn_59347177-aa7e-d8ad-9f4f-d09628b32ec3",
        "parent_transaction": "txn_0b59f6e6-6c4a-4efa-915c-526f77ef61ab",
        "status": "APPLIED"
      }
    }
  ],
...
| :--- | :--- | :--- |
| **Specific amount** | Fixed amount to send. Use `precise_distribution` when the transaction uses `precise_amount`. | `"precise_distribution": "1000000"` |
...
Blnk automatically generates unique references for each transaction record by appending a counter to your original reference (starting from `1`). From 0.15.4 the suffix matches the order of the `destinations` array and is stable across retries.
