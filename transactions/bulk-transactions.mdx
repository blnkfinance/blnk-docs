...
```json 
{
  "event": "bulk_transaction.applied | bulk_transaction.inflight | bulk_transaction.failed",
  "data": {
    "batch_id": "bulk_c62f200b-905f-4983-a349-cadd279234aa",
    "status": "applied|inflight|failed",
    "transaction_count": 4,       // Only included for success cases
    "error": "error message",     // Only included for failure cases
...
When using `atomic: false`, if any transaction in the batch fails, only that specific transaction fails while all other valid transactions are still processed. You'll receive an error response (or webhook for async) like this:
...
4. For large transaction batches (1000+ transactions), using `run_async: true` is recommended to avoid timeout issues.
