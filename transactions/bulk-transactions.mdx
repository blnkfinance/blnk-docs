...
To preview a batch without writing anything, set `dry_run` to `true`. See [Dry-run transactions](/transactions/dry-run).
...
<Note>
From Blnk Core **0.10.5**, bulk create defaults to `skip_queue: false` and enqueues each item. Before 0.10.5, every item was processed synchronously in the request.
</Note>
...
<RelatedTopics
  items={[
    ...
    { title: "0.10.5 migration", href: "/changelog/v10-5-migration" },
