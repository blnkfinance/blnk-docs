...
```json 200 Skipped queue {3,4} wrap
{
...
transaction, resp, err := client.Transaction.Update(
    ...
    blnkgo.UpdateStatus{
        Status: blnkgo.InflightStatusCommit,
        PreciseAmount: big.NewInt(4000),
        SkipQueue: false,
    },
)
```

```python Python wrap {5}
response = blnk.transactions.update_status(
  "{transaction_id}",
  {
    "status": "commit",
...
```json transaction.applied {2,4,5,14,19} wrap
{
...
```typescript TypeScript wrap {6,7,11,12}
...
ApiResponse<JsonNode> response = blnk.search().filter(
    FilterParams.create()
        .filters(List.of(
            ...
            .field("status")
            .operator("in")
            .values(List.of("APPLIED", "VOID"))
        )),
    "transactions");
```
</CodeGroup>

```json Response wrap {19,39}
{
  "data": [
    {
      "transaction_id": "txn_b2c3d4e5-f6a7-8901-bcde-f23456789012",
...
Both children share the same `parent_transaction`: the inflight transaction ID (`txn_6164573b-6cc8-45a4-ad2e-7b4ba6a60f7d`). That becomes your filter key and the direct link to the inflight transaction.
