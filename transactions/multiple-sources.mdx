...
## Multiple sources with inflight

You can initiate inflight transactions from multiple sources to a single destination. To do so, enable inflight in your transaction request. 
...
## Refund transaction with multiple sources

To bulk refund similar initiated with multiple sources, call the **refund transaction endpoint**, and pass the parent transaction id. 

```
POST http://YOUR_BLNK_INSTANCE_URL/refund-transaction/{transaction_id}
```

This automatically refunds all of the transactions from the destination to their respective sources. 
