...
<Warning>
When sending from multiple sources, you can only send to one destination. A payload that includes both `sources[]` and `destinations[]` is rejected with `400 TXN_VALIDATION_ERROR`.
</Warning>
...
Blnk automatically generates unique references for each transaction record by appending a counter to your original reference (starting from `1`). From 0.15.4 the suffix matches the order of the `sources` array and is stable across retries.
